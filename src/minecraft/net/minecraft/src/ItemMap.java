...
public class ItemMap extends ItemMapBase {
	...
	public void updateMapData(World world, Entity entity, MapData mapdata) {
		...
		int l = MathHelper.floor_double(entity.posX - j) / i + c / 2;
		int i1 = MathHelper.floor_double(entity.posZ - k) / i + c1 / 2;
		int j1 = 128 / i;
		...
		for(int k1 = (l - j1) + 1; k1 < l + j1; k1++) {
			...
			for(int j2 = i1 - j1 - 1; j2 < i1 + j1; j2++) {
				...
				int l3 = 0;
				int i4 = 0;
				int ai[] = new int[Block.blocksLength];
				...
				double d2 = ((d1 - d) * 4D) / (i + 4) + ((k1 + j2 & 1) - 0.5D) * 0.40000000000000002D;
				byte byte0 = 1;
				...
				if(i6 > 0) {
					...
					if(mapcolor == MapColor.waterColor) {
						double d3 = l4 * 0.10000000000000001D + (k1 + j2 & 1) * 0.20000000000000001D;
						byte0 = 1;
						if(d3 < 0.5D) {
							byte0 = 2;
						}
