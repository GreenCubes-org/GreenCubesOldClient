...
public class MapData extends WorldSavedData {
	...
	public void func_28169_a(EntityPlayer entityplayer, ItemStack itemstack) {
		...
		for(int i = 0; i < field_28174_h.size(); i++) {
			...
			float f = (float) (mapinfo1.entityplayerObj.posX - xCenter) / (1 << scale);
			float f1 = (float) (mapinfo1.entityplayerObj.posZ - zCenter) / (1 << scale);
			int j = 64;
			int k = 64;
			if(f < (-j) || f1 < (-k) || f > j || f1 > k) {
				continue;
			}
			byte byte0 = 0;
			byte byte1 = (byte) (int) ((f * 2.0F) + 0.5D);
			byte byte2 = (byte) (int) ((f1 * 2.0F) + 0.5D);
			byte byte3 = (byte) (int) (((entityplayer.rotationYaw * 16F) / 360F) + 0.5D);
			if(dimension < 0) {
