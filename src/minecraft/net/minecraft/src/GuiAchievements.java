...
public class GuiAchievements extends GuiScreen {
	...
	protected void genAchievementBackground(int i, int j, float f) {
		int k = MathHelper.floor_double(field_27116_m + (guiMapX - field_27116_m) * f);
		int l = MathHelper.floor_double(field_27115_n + (guiMapY - field_27115_n) * f);
		if(k < guiMapTop) {
			k = guiMapTop;
		}
		if(l < guiMapLeft) {
		...
		for(int l2 = 0; l2 < AchievementList.achievementList.size(); l2++) {
			...
			char c = Math.sin(((System.currentTimeMillis() % 600L) / 600D) * 3.1415926535897931D * 2D) <= 0.59999999999999998D ? '\202' : '\377';
			if(flag) {
				l5 = 0xff707070;
			} else if(flag1) {
				l5 = 65280 + (c << 24);
		...
		for(int l3 = 0; l3 < AchievementList.achievementList.size(); l3++) {
			...
			if(statFileWriter.hasAchievementUnlocked(achievement3)) {
				...
				float f2 = Math.sin(((System.currentTimeMillis() % 600L) / 600D) * 3.1415926535897931D * 2D) >= 0.59999999999999998D ? 0.8F : 0.6F;
		...
		if(achievement != null) {
			...
			if(statFileWriter.canUnlockAchievement(achievement2)) {
				...
				String s2 = StatCollector.translateToLocalFormatted("achievement.requires", new Object[]{achievement2.parentAchievement.statName});
