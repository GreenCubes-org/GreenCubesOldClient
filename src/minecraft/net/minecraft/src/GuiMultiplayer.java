...
public class GuiMultiplayer extends GuiScreen {
	...
	private void loadServerList() {
		...
		try {
			...
			while(sc.hasNextLine()) {
				...
				if(split[0].trim().equals("servers")) {
					if(split.length > 1) {
						System.out.println(line);
						String[] sSplit = line.substring(split[0].length() + 1).split(";");
						for(int i = 0; i < sSplit.length; ++i) {
							System.out.println(sSplit[i]);
							String[] srv = sSplit[i].split(",");
							if(srv.length > 1) {
								gcs.add(srv[1].trim());
								serverList.add(new ServerNBTStorage(srv[0].trim(), srv[1].trim()));
							}
						}
					}
				}
		...
		try {
			...
			// GreenCubes start
			all: for(int i = 0; i < nbttaglist.size(); i++) {
				for(int n = 0; n < gcs.size(); ++n) {
					if(gcs.get(n).equalsIgnoreCase(((NBTTagCompound) nbttaglist.get(i)).getString("ip"))) {
						continue all;
					}
				}
				serverList.add(ServerNBTStorage.createServerNBTStorage((NBTTagCompound) nbttaglist.get(i)));
			}
			// GreenCubes end
	...
	static void pollServer(GuiMultiplayer guimultiplayer, ServerNBTStorage servernbtstorage) throws IOException {
