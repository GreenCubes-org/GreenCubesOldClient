...
public class InventoryPlayer implements IInventory {
	...
	public NBTTagList writeToNBT(NBTTagList nbttaglist) {
		...
		for(int j = 0; j < decorInventory.length; j++) {
			if(decorInventory[j] != null) {
				NBTTagCompound nbttagcompound1 = new NBTTagCompound();
				nbttagcompound1.setByte("Slot", (byte) (j + 200));
				decorInventory[j].writeToNBT(nbttagcompound1);
				nbttaglist.setTag(nbttagcompound1);
			}
		}
		for(int j = 0; j < trashcan.length; j++) {
			if(trashcan[j] != null) {
				NBTTagCompound nbttagcompound1 = new NBTTagCompound();
				nbttagcompound1.setByte("Slot", (byte) (j + 255));
				trashcan[j].writeToNBT(nbttagcompound1);
				nbttaglist.setTag(nbttagcompound1);
			}
		}
	...
	public ItemStack armorItemInSlot(int i) {
		return decorInventory[i] != null ? decorInventory[i] : armorInventory[i];
	}
