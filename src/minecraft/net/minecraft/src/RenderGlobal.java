...
public class RenderGlobal implements IWorldAccess {
	...
	public RenderGlobal(Minecraft minecraft, RenderEngine renderengine) {
		...
		byte byte0 = 34;
		byte byte1 = 8;
		glRenderListBase = GLAllocation.generateDisplayLists(byte0 * byte0 * byte1 * 4);
		...
		tessellator.draw();
		GL11.glEndList();
		TileEntityRenderer.instance.init();
	}
	...
	private void renderStars() {
		...
		for(int i = 0; i < 1500; i++) {
			...
			double d3 = 0.25F + random.nextFloat() * 0.1F;
			...
			double d6 = d1 * 100D;
			double d7 = d2 * 100D;
			double d8 = TrigMath.atan2(d, d2);
			double d9 = Math.sin(d8);
			double d10 = Math.cos(d8);
			double d11 = TrigMath.atan2(Math.sqrt(d * d + d2 * d2), d1);
	...
	public void renderEntities(Vec3D vec3d, Frustrum icamera, float f) {
		if(renderEntitiesStartupCounter > 0) {
			renderEntitiesStartupCounter--;
			return;
		}
		if(renderEngine == null || mc.fontRenderer == null || mc.renderViewEntity == null || mc.gameSettings == null)
			return;
		...
		EntityLiving entityliving = mc.renderViewEntity;
		RenderManager.renderPosX = ((Entity) (entityliving)).lastTickPosX + (((Entity) (entityliving)).posX - ((Entity) (entityliving)).lastTickPosX) * f;
		RenderManager.renderPosY = ((Entity) (entityliving)).lastTickPosY + (((Entity) (entityliving)).posY - ((Entity) (entityliving)).lastTickPosY) * f;
		RenderManager.renderPosZ = ((Entity) (entityliving)).lastTickPosZ + (((Entity) (entityliving)).posZ - ((Entity) (entityliving)).lastTickPosZ) * f;
		TileEntityRenderer.staticPlayerX = ((Entity) (entityliving)).lastTickPosX + (((Entity) (entityliving)).posX - ((Entity) (entityliving)).lastTickPosX) * f;
		TileEntityRenderer.staticPlayerY = ((Entity) (entityliving)).lastTickPosY + (((Entity) (entityliving)).posY - ((Entity) (entityliving)).lastTickPosY) * f;
	...
	public String getDebugInfoRenders() {
		return (new StringBuilder()).append("C: ").append(renderersBeingRendered).append('/').append(renderersLoaded).append(". F: ").append(renderersBeingClipped).append(", O: ").append(renderersBeingOccluded).append(", E: ").append(renderersSkippingRenderPass).append(". T: ").append(tilesRendered).append('/').append(tileEntities.size()).append(". OCC: ").append(ARBOcclusionChecker.instance.hiddenObjects).append('/').append(ARBOcclusionChecker.instance.objects).toString();
	}
	...
	public String getDebugInfoTiles() {
		int signs = 0;
		int mobs = 0;
		int pistons = 0;
		int chests = 0;
		for(int k = 0; k < tileEntities.size(); k++) {
			...
			if(t instanceof TileEntityChest)
				chests++;
			else if(t instanceof TileEntityMobSpawner)
				mobs++;
			else if(t instanceof TileEntityPiston)
				pistons++;
			else if(t instanceof TileEntitySign)
				signs++;
		}
		return new StringBuilder().append("S: ").append(signs).append(", C: ").append(chests).append(", P: ").append(pistons).append(", M: ").append(mobs).toString();
	}
	...
	public int sortAndRender(EntityLiving entityliving, int i, double d) {
		...
		if(occlusionEnabled && mc.gameSettings.advancedOpengl && !mc.gameSettings.anaglyph && i == 0) {
			...
			do {
				...
				for(int k1 = byte0; k1 < i1; k1++) {
					...
					float f4 = (float) (worldrenderer1.posXMinus - d1);
					float f5 = (float) (worldrenderer1.posYMinus - d2);
					float f6 = (float) (worldrenderer1.posZMinus - d3);
					float f7 = f4 - f;
					float f8 = f5 - f1;
					float f9 = f6 - f2;
		...
		if(i == 0 && ARBOcclusionChecker.instance.checkEnabled()) {
			Profiler.endStartSection("occ");
			ARBOcclusionChecker.instance.tick();
		}
		Profiler.endSection();
		return k;
	}
	...
	public void renderSky(float f) {
		...
		GL11.glRotatef(0.0F, 0.0F, 1.0F, 0.0F); // GreenCubes: BACK TO NORMAL!!!
	...
	public void renderClouds(float f) {
		...
		float f1 = (float) (mc.renderViewEntity.lastTickPosY + (mc.renderViewEntity.posY - mc.renderViewEntity.lastTickPosY) * f);
		byte byte0 = 32;
		int i = 256 / byte0;
		Tessellator tessellator = Tessellator.instance;
		...
		double d = cloudOffsetX + f;
		double d1 = mc.renderViewEntity.prevPosX + (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * f + d * 0.029999999329447746D;
		double d2 = mc.renderViewEntity.prevPosZ + (mc.renderViewEntity.posZ - mc.renderViewEntity.prevPosZ) * f;
		...
		for(int l = -byte0 * i; l < byte0 * i; l += byte0) {
			for(int i1 = -byte0 * i; i1 < byte0 * i; i1 += byte0) {
				tessellator.addVertexWithUV(l + 0, f9, i1 + byte0, (l + 0) * f6 + f10, (i1 + byte0) * f6 + f11);
				tessellator.addVertexWithUV(l + byte0, f9, i1 + byte0, (l + byte0) * f6 + f10, (i1 + byte0) * f6 + f11);
				tessellator.addVertexWithUV(l + byte0, f9, i1 + 0, (l + byte0) * f6 + f10, (i1 + 0) * f6 + f11);
				tessellator.addVertexWithUV(l + 0, f9, i1 + 0, (l + 0) * f6 + f10, (i1 + 0) * f6 + f11);
			}
	...
	public void renderCloudsFancy(float f) {
		...
		float f3 = 4F;
		double d = cloudOffsetX + f;
		double d1 = (mc.renderViewEntity.prevPosX + (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * f + d * 0.029999999329447746D) / f2;
		double d2 = (mc.renderViewEntity.prevPosZ + (mc.renderViewEntity.posZ - mc.renderViewEntity.prevPosZ) * f) / f2 + 0.33000001311302185D;
		...
		f9 = MathHelper.floor_double(d1) * f13;
		f11 = MathHelper.floor_double(d2) * f13;
		float f14 = (float) (d1 - MathHelper.floor_double(d1));
		float f15 = (float) (d2 - MathHelper.floor_double(d2));
		int k = 8;
		byte byte0 = 4;
			...
			for(int i1 = -byte0 + 1; i1 <= byte0; i1++) {
				for(int j1 = -byte0 + 1; j1 <= byte0; j1++) {
					...
					if(f4 > -f3 - 1.0F) {
						...
						tessellator.addVertexWithUV(f19 + 0.0F, f4 + 0.0F, f20 + k, (f17 + 0.0F) * f13 + f9, (f18 + k) * f13 + f11);
						tessellator.addVertexWithUV(f19 + k, f4 + 0.0F, f20 + k, (f17 + k) * f13 + f9, (f18 + k) * f13 + f11);
						tessellator.addVertexWithUV(f19 + k, f4 + 0.0F, f20 + 0.0F, (f17 + k) * f13 + f9, (f18 + 0.0F) * f13 + f11);
					...
					if(f4 <= f3 + 1.0F) {
						...
						tessellator.addVertexWithUV(f19 + 0.0F, (f4 + f3) - f16, f20 + k, (f17 + 0.0F) * f13 + f9, (f18 + k) * f13 + f11);
						tessellator.addVertexWithUV(f19 + k, (f4 + f3) - f16, f20 + k, (f17 + k) * f13 + f9, (f18 + k) * f13 + f11);
						tessellator.addVertexWithUV(f19 + k, (f4 + f3) - f16, f20 + 0.0F, (f17 + k) * f13 + f9, (f18 + 0.0F) * f13 + f11);
					...
					if(i1 > -1) {
						...
						for(int k1 = 0; k1 < k; k1++) {
							tessellator.addVertexWithUV(f19 + k1 + 0.0F, f4 + 0.0F, f20 + k, (f17 + k1 + 0.5F) * f13 + f9, (f18 + k) * f13 + f11);
							tessellator.addVertexWithUV(f19 + k1 + 0.0F, f4 + f3, f20 + k, (f17 + k1 + 0.5F) * f13 + f9, (f18 + k) * f13 + f11);
							tessellator.addVertexWithUV(f19 + k1 + 0.0F, f4 + f3, f20 + 0.0F, (f17 + k1 + 0.5F) * f13 + f9, (f18 + 0.0F) * f13 + f11);
							tessellator.addVertexWithUV(f19 + k1 + 0.0F, f4 + 0.0F, f20 + 0.0F, (f17 + k1 + 0.5F) * f13 + f9, (f18 + 0.0F) * f13 + f11);
						}
					...
					if(i1 <= 1) {
						...
						for(int l1 = 0; l1 < k; l1++) {
							tessellator.addVertexWithUV((f19 + l1 + 1.0F) - f16, f4 + 0.0F, f20 + k, (f17 + l1 + 0.5F) * f13 + f9, (f18 + k) * f13 + f11);
							tessellator.addVertexWithUV((f19 + l1 + 1.0F) - f16, f4 + f3, f20 + k, (f17 + l1 + 0.5F) * f13 + f9, (f18 + k) * f13 + f11);
							tessellator.addVertexWithUV((f19 + l1 + 1.0F) - f16, f4 + f3, f20 + 0.0F, (f17 + l1 + 0.5F) * f13 + f9, (f18 + 0.0F) * f13 + f11);
							tessellator.addVertexWithUV((f19 + l1 + 1.0F) - f16, f4 + 0.0F, f20 + 0.0F, (f17 + l1 + 0.5F) * f13 + f9, (f18 + 0.0F) * f13 + f11);
						}
					...
					if(j1 > -1) {
						...
						for(int i2 = 0; i2 < k; i2++) {
							tessellator.addVertexWithUV(f19 + 0.0F, f4 + f3, f20 + i2 + 0.0F, (f17 + 0.0F) * f13 + f9, (f18 + i2 + 0.5F) * f13 + f11);
							tessellator.addVertexWithUV(f19 + k, f4 + f3, f20 + i2 + 0.0F, (f17 + k) * f13 + f9, (f18 + i2 + 0.5F) * f13 + f11);
							tessellator.addVertexWithUV(f19 + k, f4 + 0.0F, f20 + i2 + 0.0F, (f17 + k) * f13 + f9, (f18 + i2 + 0.5F) * f13 + f11);
							tessellator.addVertexWithUV(f19 + 0.0F, f4 + 0.0F, f20 + i2 + 0.0F, (f17 + 0.0F) * f13 + f9, (f18 + i2 + 0.5F) * f13 + f11);
						}
					...
					if(j1 <= 1) {
						...
						for(int j2 = 0; j2 < k; j2++) {
							tessellator.addVertexWithUV(f19 + 0.0F, f4 + f3, (f20 + j2 + 1.0F) - f16, (f17 + 0.0F) * f13 + f9, (f18 + j2 + 0.5F) * f13 + f11);
							tessellator.addVertexWithUV(f19 + k, f4 + f3, (f20 + j2 + 1.0F) - f16, (f17 + k) * f13 + f9, (f18 + j2 + 0.5F) * f13 + f11);
							tessellator.addVertexWithUV(f19 + k, f4 + 0.0F, (f20 + j2 + 1.0F) - f16, (f17 + k) * f13 + f9, (f18 + j2 + 0.5F) * f13 + f11);
							tessellator.addVertexWithUV(f19 + 0.0F, f4 + 0.0F, (f20 + j2 + 1.0F) - f16, (f17 + 0.0F) * f13 + f9, (f18 + j2 + 0.5F) * f13 + f11);
						}
	...
	public boolean updateRenderers(EntityLiving entityliving, boolean flag) {
		...
		Profiler.endStartSection("queue");
		byte byte0 = 2;
		...
		Profiler.endStartSection("update2");
		if(arraylist != null) {
			if(arraylist.size() > 1) {
		...
		Profiler.endStartSection("queue2");
		int j2 = 0;
		int l2 = 0;
	...
	public void drawBlockBreaking(EntityPlayer entityplayer, MovingObjectPosition movingobjectposition, int i, ItemStack itemstack, float f) {
		...
		if(k == Block.snow.blockID)
			return;
		Tessellator tessellator = Tessellator.instance;
		GL11.glEnable(3042 /* GL_BLEND */);
		GL11.glEnable(3008 /* GL_ALPHA_TEST */);
		GL11.glBlendFunc(770, 1);
		GL11.glColor4f(1.0F, 1.0F, 1.0F, (MathHelper.sin(System.currentTimeMillis() / 100F) * 0.2F + 0.4F) * 0.5F);
		if(i == 0) {
			if(damagePartialTime > 0.0F) {
				...
				double d1 = entityplayer.lastTickPosY + (entityplayer.posY - entityplayer.lastTickPosY) * f;
				double d2 = entityplayer.lastTickPosZ + (entityplayer.posZ - entityplayer.lastTickPosZ) * f;
				if(block == null) {
					block = Block.stone;
				}
				GL11.glEnable(3008 /* GL_ALPHA_TEST */);
			...
			GL11.glBlendFunc(770, 771);
			float f1 = MathHelper.sin(System.currentTimeMillis() / 100F) * 0.2F + 0.8F;
			GL11.glColor4f(f1, f1, f1, MathHelper.sin(System.currentTimeMillis() / 200F) * 0.2F + 0.5F);
	...
	public void drawSelectionBox(EntityPlayer entityplayer, MovingObjectPosition movingobjectposition, int i, ItemStack itemstack, float f) {
		if(i == 0 && movingobjectposition.typeOfHit == EnumMovingObjectType.TILE) {
			if(itemstack != null) {
				...
				if(item != null && item.shouldDrawOnSelection(itemstack))
					item.drawOnSelection(worldObj, movingobjectposition, itemstack, entityplayer, f);
			}
			GL11.glEnable(3042 /* GL_BLEND */);
			GL11.glBlendFunc(770, 771);
	...
	public void drawPoint(Vec3D vec) {
		Tessellator tessellator = Tessellator.instance;
		tessellator.startDrawing(3);
		tessellator.addVertex(vec.xCoord, vec.yCoord - 0.05, vec.zCoord);
		tessellator.addVertex(vec.xCoord, vec.yCoord + 0.05, vec.zCoord);
		tessellator.draw();
		tessellator.startDrawing(3);
		tessellator.addVertex(vec.xCoord - 0.05, vec.yCoord, vec.zCoord);
		tessellator.addVertex(vec.xCoord + 0.05, vec.yCoord, vec.zCoord);
		tessellator.draw();
		tessellator.startDrawing(3);
		tessellator.addVertex(vec.xCoord, vec.yCoord, vec.zCoord - 0.05);
		tessellator.addVertex(vec.xCoord, vec.yCoord, vec.zCoord + 0.05);
		tessellator.draw();
	}
	...
	public boolean isBlockVisible(int x, int y, int z) {
		int k1 = MathHelper.bucketInt(x, 16);
		int l1 = MathHelper.bucketInt(y, 16);
		int i2 = MathHelper.bucketInt(z, 16);
		int j3 = k1 % renderChunksWide;
		if(j3 < 0)
			j3 += renderChunksWide;
		int l3 = l1 % renderChunksTall;
		if(l3 < 0)
			l3 += renderChunksTall;
		int j4 = i2 % renderChunksDeep;
		if(j4 < 0)
			j4 += renderChunksDeep;
		int k4 = (j4 * renderChunksTall + l3) * renderChunksWide + j3;
		WorldRenderer worldrenderer = worldRenderers[k4];
		return worldrenderer.isInFrustum && worldrenderer.isVisible;
	}
			...
			for(int k3 = l1; k3 <= k2; k3++) {
				...
				for(int i4 = i2; i4 <= l2; i4++) {
					...
					if(!worldrenderer.needsUpdate) {
						...
						//worldrenderer.lastUpdateCause = i + "," + j + "," + k + " to " + l + "," + i1 + "," + j1;
					}
				}
	...
	public void playSound(String s, double d, double d1, double d2, float f, float f1) {
		...
		if(mc.renderViewEntity.getDistanceSq(d, d1, d2) < (f2 * f2)) {
	...
	public EntityFXGC func_40193_b(String s, double d, double d1, double d2, double d3, double d4, double d5) {
		...
		if(s.equals("hugeexplosion")) {
			mc.effectRenderer.addEffect((obj = new EntityHugeExplodeFXGC(worldObj, d, d1, d2, d3, d4, d5)));
		} else if(s.equals("largeexplode")) {
			mc.effectRenderer.addEffect((obj = new EntityLargeExplodeFXGC(renderEngine, worldObj, d, d1, d2, d3, d4, d5)));
		} else if (s.equals("fireworksSpark")) {
            this.mc.effectRenderer.addEffect(obj = new EntityFireworkSparkFXGC(this.worldObj, d, d1, d2, d3, d4, d5));
        }
		if(obj != null)
			return obj;
		double d9 = 16D;
		...
		if(i > 1)
			return null;
		if(s.equals("bubble")) {
			obj = new EntityBubbleFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("suspended")) {
			obj = new EntitySuspendFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("depthsuspend")) {
			obj = new EntityAuraFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("townaura")) {
			obj = new EntityAuraFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("crit")) {
			obj = new EntityCritFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("magicCrit")) {
			obj = new EntityCritFXGC(worldObj, d, d1, d2, d3, d4, d5);
			obj.setColor(obj.getRedColor() * 0.3F, obj.getGreenColor() * 0.8F, obj.getBlueColor());
			obj.setTextureIndex(obj.getTextureIndex() + 1);
		} else if(s.equals("smoke")) {
			obj = new EntitySmokeFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("mobSpell")) {
			obj = new EntitySpellParticleFXGC(worldObj, d, d1, d2, 0.0D, 0.0D, 0.0D);
			obj.setColor((float) d3, (float) d4, (float) d5);
		} else if(s.equals("spell")) {
			obj = new EntitySpellParticleFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("instantSpell")) {
			obj = new EntitySpellParticleFXGC(worldObj, d, d1, d2, d3, d4, d5);
			((EntitySpellParticleFXGC) obj).func_40110_b(144);
		} else if(s.equals("note")) {
			obj = new EntityNoteFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("portal")) {
			obj = new EntityPortalFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("enchantmenttable")) {
			obj = new EntityEnchantmentTableParticleFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("explode")) {
			obj = new EntityExplodeFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("flame")) {
			obj = new EntityFlameFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("lava")) {
			obj = new EntityLavaFXGC(worldObj, d, d1, d2);
		} else if(s.equals("footstep")) {
			obj = new EntityFootStepFXGC(renderEngine, worldObj, d, d1, d2);
		} else if(s.equals("splash")) {
			obj = new EntitySplashFX(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("largesmoke")) {
			obj = new EntitySmokeFXGC(worldObj, d, d1, d2, d3, d4, d5, 2.5F);
		} else if(s.equals("cloud")) {
			obj = new EntityCloudFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("reddust")) {
			obj = new EntityReddustFXGC(worldObj, d, d1, d2, (float) d3, (float) d4, (float) d5);
		} else if(s.equals("bluedust")) {
			obj = new EntityBluedustFXGC(worldObj, d, d1, d2, (float) d3, (float) d4, (float) d5);
		} else if(s.equals("sakura")) {
			obj = new EntitySakuraFXGC(worldObj, d, d1, d2, (float) d3, (float) d4, (float) d5);
		} else if(s.equals("snowballpoof")) {
			obj = new EntityBreakingFXGC(worldObj, d, d1, d2, Item.snowball);
		} else if(s.equals("dripWater")) {
			obj = new EntityDropParticleFXGC(worldObj, d, d1, d2, Material.water);
		} else if(s.equals("dripLava")) {
			obj = new EntityDropParticleFXGC(worldObj, d, d1, d2, Material.lava);
		} else if(s.equals("snowshovel")) {
			obj = new EntitySnowShovelFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("slime")) {
			obj = new EntityBreakingFXGC(worldObj, d, d1, d2, Item.dyePowder);
		} else if(s.equals("heart")) {
			obj = new EntityHeartFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.equals("dot")) {
			obj= new EntityDotFXGC(worldObj, d, d1, d2, d3, d4, d5);
		} else if(s.startsWith("iconcrack_")) {
			int j = Integer.parseInt(s.substring(s.indexOf("_") + 1));
			obj = new EntityBreakingFXGC(worldObj, d, d1, d2, d3, d4, d5, Item.itemsList[j]);
		} else if(s.startsWith("tilecrack_")) {
			int k = Integer.parseInt(s.substring(s.indexOf("_") + 1));
			obj = new EntityDiggingFXGC(worldObj, d, d1, d2, d3, d4, d5, Block.blocksList[k], 0, 0);
		}
		if(obj != null) {
	...
	public void playAuxSFX(EntityPlayer entityplayer, int i, int j, int k, int l, int i1) {
		...
		case 2000:
			int j1 = i1 % 3 - 1;
			int i2 = (i1 / 3) % 3 - 1;
			double d3 = j + j1 * 0.59999999999999998D + 0.5D;
			double d7 = k + 0.5D;
			double d11 = l + i2 * 0.59999999999999998D + 0.5D;
		...
		case 2002:
			...
			float f = (l2 >> 16 & 0xff) / 255F;
			float f1 = (l2 >> 8 & 0xff) / 255F;
			float f2 = (l2 >> 0 & 0xff) / 255F;
			String s2 = "spell";
			...
			for(int j3 = 0; j3 < 100; j3++) {
				...
				EntityFXGC entityfx = func_40193_b(s2, d1 + d20 * 0.10000000000000001D, d5 + 0.29999999999999999D, d9 + d24 * 0.10000000000000001D, d20, d22, d24);
				if(entityfx != null) {
			...
			worldObj.playSoundEffect(j + 0.5D, k + 0.5D, l + 0.5D, "random.glass", 1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F);
			break;

		case 2001:
			int k1 = i1 & 4095; // GreenCubes: 256 -> 4095
			if(k1 > 0) {
				Block block = Block.blocksList[k1];
				mc.sndManager.playSound(block.stepSound.stepSoundDir(), j + 0.5F, k + 0.5F, l + 0.5F, (block.stepSound.getVolume() + 1.0F) / 2.0F, block.stepSound.getPitch() * 0.8F);
			}
			mc.effectRenderer.addBlockDestroyEffects(j, k, l, i1 & 4095, i1 >> 12 & 0xff);
			break;

		case 2004:
			for(int l1 = 0; l1 < 20; l1++) {
				double d2 = j + 0.5D + (worldObj.rand.nextFloat() - 0.5D) * 2D;
				double d6 = k + 0.5D + (worldObj.rand.nextFloat() - 0.5D) * 2D;
				double d10 = l + 0.5D + (worldObj.rand.nextFloat() - 0.5D) * 2D;
			...
			break;
		case 2005:
			k1 = i1 & 4095; // GreenCubes: 256 -> 4095
			if(k1 > 0) {
				Block block = Block.blocksList[k1];
				mc.sndManager.playSound(block.stepSound.stepSoundDir2(), j + 0.5F, k + 0.5F, l + 0.5F, (block.stepSound.getVolume() + 1.0F) / 2.0F, block.stepSound.getPitch() * 0.8F);
			}
			break;

		case 1003:
			if(Math.random() < 0.5D) {
				worldObj.playSoundEffect(j + 0.5D, k + 0.5D, l + 0.5D, "random.door_open", 1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F);
			} else {
				worldObj.playSoundEffect(j + 0.5D, k + 0.5D, l + 0.5D, "random.door_close", 1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F);
			}
			break;

		case 1004:
			worldObj.playSoundEffect(j + 0.5F, k + 0.5F, l + 0.5F, "random.fizz", 0.5F, 2.6F + (random.nextFloat() - random.nextFloat()) * 0.8F);
			break;
		...
		case 1007:
			worldObj.playSoundEffect(j + 0.5D, k + 0.5D, l + 0.5D, "mob.ghast.charge", 10F, (random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F);
			break;

		case 1008:
			worldObj.playSoundEffect(j + 0.5D, k + 0.5D, l + 0.5D, "mob.ghast.fireball", 10F, (random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F);
			break;

		case 1009:
			worldObj.playSoundEffect(j + 0.5D, k + 0.5D, l + 0.5D, "mob.ghast.fireball", 1.0F, (random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F);
			break;
		}
	}
}
