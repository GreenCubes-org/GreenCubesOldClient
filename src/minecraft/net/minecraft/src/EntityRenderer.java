...
public class EntityRenderer {
	...
	public EntityRenderer(Minecraft minecraft) {
		...
		lightmapTexture = minecraft.renderEngine.allocateAndSetupTexture(new BufferedImage(16, 16, 1));
	...
	public void updateRenderer() {
		...
		float f3 = (3 - mc.gameSettings.renderDistance) / 3F;
	...
	public void getMouseOver(float f) {
		...
		mc.ingameGUI.itemStackDescription = null;
		...
		if(mc.objectMouseOver != null && mc.objectMouseOver.typeOfHit == EnumMovingObjectType.TILE) {
			TileEntity t = mc.theWorld.getBlockTileEntity(mc.objectMouseOver.blockX, mc.objectMouseOver.blockY, mc.objectMouseOver.blockZ);
			if(t != null && t instanceof TileEntityFrame)
				mc.ingameGUI.itemStackDescription = ((TileEntityFrame) t).getStackInSlot(0);
		}
	}
	...
	private float getFOVModifier(float f, boolean flag) {
		...
		if(entityplayer.getEntityHealth() <= 0) {
			float f2 = entityplayer.deathTime + f;
	...
	private void hurtCameraEffect(float f) {
		...
		float f1 = entityliving.hurtTime - f;
		if(entityliving.getEntityHealth() <= 0) {
			float f2 = entityliving.deathTime + f;
	...
	private void orientCamera(float f) {
		...
		double d1 = (entityliving.prevPosY + (entityliving.posY - entityliving.prevPosY) * f) - f1;
		double d2 = entityliving.prevPosZ + (entityliving.posZ - entityliving.prevPosZ) * f;
		...
		if(entityliving.isPlayerSleeping()) {
			...
			if(mc.gameSettings.debugCamEnable) {
				...
				double d4 = (-MathHelper.sin((f3 / 180F) * 3.141593F) * MathHelper.cos((f5 / 180F) * 3.141593F)) * d3;
				double d5 = (MathHelper.cos((f3 / 180F) * 3.141593F) * MathHelper.cos((f5 / 180F) * 3.141593F)) * d3;
				double d6 = (-MathHelper.sin((f5 / 180F) * 3.141593F)) * d3;
				for(int l = 0; l < 8; l++) {
					float f6 = (l & 1) * 2 - 1;
					float f7 = (l >> 1 & 1) * 2 - 1;
					float f8 = (l >> 2 & 1) * 2 - 1;
					...
					f7 *= 0.1F;
					f8 *= 0.1F;
					MovingObjectPosition movingobjectposition = mc.theWorld.rayTraceBlocks(Vec3D.createVector(d + f6, d1 + f7, d2 + f8), Vec3D.createVector((d - d4) + f6 + f8, (d1 - d6) + f7, (d2 - d5) + f8));
					if(movingobjectposition == null) {
						continue;
					}
		...
		d = entityliving.prevPosX + (entityliving.posX - entityliving.prevPosX) * f;
		d1 = (entityliving.prevPosY + (entityliving.posY - entityliving.prevPosY) * f) - f1;
		d2 = entityliving.prevPosZ + (entityliving.posZ - entityliving.prevPosZ) * f;
	...
	private void setupCameraTransform(float f, int i) {
		...
		if(f3 > 0.0F) {
			...
			f4 *= f4;
			GL11.glRotatef((rendererUpdateCount + f) * j, 0.0F, 1.0F, 1.0F);
	...
	private void updateLightmap() {
		...
		bi.setRGB(0, 0, 16, 16, lightmapColors, 0, 16);
		try {
			ImageIO.write(bi, "png", new File("lightmap.png"));
		} catch (IOException e) {
	...
	public void updateCameraAndRender(float f) {
		...
		if(mc.inGameHasFocus) {
			...
			if(mc.gameSettings.smoothCamera) {
				...
				mc.thePlayer.setAngles(f3, f4 * l);
			} else {
				mc.thePlayer.setAngles(f3, f4 * l);
			}
		}
		...
		if(mc.theWorld != null) {
			...
			if(mc.gameSettings.limitFramerate == 0) {
				...
				renderWorld(f, renderEndNanoTime + (0x3b9aca00 / c));
			}
			Profiler.endStartSection("sleep");
			if(mc.gameSettings.limitFramerate == 2) {
				long l1 = ((renderEndNanoTime + (0x3b9aca00 / c)) - System.nanoTime()) / 0xf4240L;
				if(l1 > 0L && l1 < 500L) {
					try {
						Thread.sleep(l1);
			...
			if(mc.gameSettings.limitFramerate == 2) {
				long l2 = ((renderEndNanoTime + (0x3b9aca00 / c)) - System.nanoTime()) / 0xf4240L;
				if(l2 < 0L) {
					l2 += 10L;
				}
				if(l2 > 0L && l2 < 500L) {
	...
	public void renderWorld(float f, long l) {
		...
		int i3 = MathHelper.floor_double(mc.renderViewEntity.posX);
		int j1 = MathHelper.floor_double(mc.renderViewEntity.posY) - 2;
		int k1 = MathHelper.floor_double(mc.renderViewEntity.posZ);
		...
		double d1 = entityliving.lastTickPosY + (entityliving.posY - entityliving.lastTickPosY) * f;
		double d2 = entityliving.lastTickPosZ + (entityliving.posZ - entityliving.lastTickPosZ) * f;
		...
		for(int i = 0; i < 2; i++) {
			...
			if(debugViewDirection == 0) {
				...
				effectrenderer.renderParticles(entityliving, frustrum, f);
				disableLightmap(f);
			...
			if(mc.gameSettings.fancyGraphics) {
				...
				// Colored glass
				GL11.glColorMask(false, false, false, false);
				i1 = renderglobal.sortAndRender(entityliving, 2, f);
				if(mc.gameSettings.anaglyph) {
					if(anaglyphField == 0) {
						GL11.glColorMask(false, true, true, true);
					} else {
				...
				if(i1 > 0)
					renderglobal.renderAllRenderLists(2, f);
				GL11.glShadeModel(7424 /*GL_FLAT*/);
			} else {
				Profiler.endStartSection("water");
	...
	private void addRainParticles() {
		...
		random.setSeed(rendererUpdateCount * 0x12a7ce5fL);
		EntityLiving entityliving = mc.renderViewEntity;
		World world = mc.theWorld;
		...
		for(int j1 = 0; j1 < i1; j1++) {
			...
			BiomeGenBase biome = world.getBiomeAt(k1, i2, l1);
			if(!biome.canSpawnLightningBolt() || biome.temperature <= 0.2F)
				continue;
			...
			if(Block.blocksList[j2].blockMaterial == Material.lava) {
				mc.effectRenderer.addEffect(new EntitySmokeFXGC(world, k1 + f1, (i2 + 0.1F) - Block.blocksList[j2].minY, l1 + f2, 0.0D, 0.0D, 0.0D));
				continue;
			}
			if(random.nextInt(++l) == 0) {
				d = k1 + f1;
				d1 = (i2 + 0.1F) - Block.blocksList[j2].minY;
				d2 = l1 + f2;
			}
			mc.effectRenderer.addEffect(new EntityRainFXGC(world, k1 + f1, (i2 + 0.1F) - Block.blocksList[j2].minY, l1 + f2));
		}

		float RAIN_BASE_SOUND = 0.5F;
		...
		if(l > 0 && random.nextInt(3) < rainSoundCounter++) {
			...
			if(d1 > entityliving.posY + 1.0D && world.func_35461_e(MathHelper.floor_double(entityliving.posX), MathHelper.floor_double(entityliving.posZ)) > MathHelper.floor_double(entityliving.posY)) {
				mc.theWorld.playSoundEffect(d, d1, d2, "ambient.weather.rain", 0.1F * RAIN_BASE_SOUND, 0.5F);
			} else {
				mc.theWorld.playSoundEffect(d, d1, d2, "ambient.weather.rain", 0.2F * RAIN_BASE_SOUND, 1.0F);
			}
		}
	}
	...
	protected void renderRainSnow(float f) {
		...
		int l1 = 0;
		byte byte0 = -1;
		float f5 = rendererUpdateCount + f;
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		l1 = 0;
		for(int i2 = i1 - k1; i2 <= i1 + k1; i2++) {
			for(int j2 = k - k1; j2 <= k + k1; j2++) {
				...
				BiomeGenBase biomegenbase = world.getBiomeAt(j2, 127, i2);
				...
				if(!biomegenbase.getEnableSnow()) {
					...
					float f10 = (((rendererUpdateCount + j2 * j2 * 3121 /*GL_RGBA_MODE*/+ j2 * 0x2b24abb + i2 * i2 * 0x66397 + i2 * 13761 & 0x1f) + f) / 32F) * (3F + random.nextFloat());
					double d3 = (j2 + 0.5F) - entityliving.posX;
					double d4 = (i2 + 0.5F) - entityliving.posZ;
					float f14 = MathHelper.sqrt_double(d3 * d3 + d4 * d4) / k1;
					float f15 = 1.0F;
					...
					tessellator.addVertexWithUV((j2 - f6) + 0.5D, i3, (i2 - f7) + 0.5D, 0.0F * f8, (i3 * f8) / 4F + f10 * f8);
					tessellator.addVertexWithUV((j2 + f6) + 0.5D, i3, (i2 + f7) + 0.5D, 1.0F * f8, (i3 * f8) / 4F + f10 * f8);
					tessellator.addVertexWithUV((j2 + f6) + 0.5D, j3, (i2 + f7) + 0.5D, 1.0F * f8, (j3 * f8) / 4F + f10 * f8);
					tessellator.addVertexWithUV((j2 - f6) + 0.5D, j3, (i2 - f7) + 0.5D, 0.0F * f8, (j3 * f8) / 4F + f10 * f8);
				...
				float f11 = ((rendererUpdateCount & 0x1ff) + f) / 512F;
				...
				double d5 = (j2 + 0.5F) - entityliving.posX;
				double d6 = (i2 + 0.5F) - entityliving.posZ;
				float f16 = MathHelper.sqrt_double(d5 * d5 + d6 * d6) / k1;
				float f17 = 1.0F;
				...
				tessellator.setTranslationD(-d * 1.0D, -d1 * 1.0D, -d2 * 1.0D);
				tessellator.addVertexWithUV((j2 - f6) + 0.5D, i3, (i2 - f7) + 0.5D, 0.0F * f8 + f12, (i3 * f8) / 4F + f11 * f8 + f13);
				tessellator.addVertexWithUV((j2 + f6) + 0.5D, i3, (i2 + f7) + 0.5D, 1.0F * f8 + f12, (i3 * f8) / 4F + f11 * f8 + f13);
				tessellator.addVertexWithUV((j2 + f6) + 0.5D, j3, (i2 + f7) + 0.5D, 1.0F * f8 + f12, (j3 * f8) / 4F + f11 * f8 + f13);
				tessellator.addVertexWithUV((j2 - f6) + 0.5D, j3, (i2 - f7) + 0.5D, 0.0F * f8 + f12, (j3 * f8) / 4F + f11 * f8 + f13);
				tessellator.setTranslationD(0.0D, 0.0D, 0.0D);
			}
	...
	private void updateFogColor(float f) {
		...
		fogColorGreen *= f11;
		fogColorBlue *= f11;
		double d = (entityliving.lastTickPosY + (entityliving.posY - entityliving.lastTickPosY) * f) / 32D;
	...
	private void setupFog(int i, float f) {
		...
		if(entityliving.isPotionActive(Potion.potionBlindness)) {
			...
			if(k < 20) {
				f1 = 5F + (farPlaneDistance - 5F) * (1.0F - k / 20F);
			}
			GL11.glFogi(2917 /*GL_FOG_MODE*/, 9729 /*GL_LINEAR*/);
			if(i < 0) {
				GL11.glFogf(2915 /*GL_FOG_START*/, 0.0F);
