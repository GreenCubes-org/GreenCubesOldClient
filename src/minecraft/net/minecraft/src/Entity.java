...
public abstract class Entity {
	...
	public Entity(World world) {
		...
		addedToChunk = false;
		worldObj = world;
		setSpawnPosition(0.0D, 0.0D, 0.0D);
	...
	public void setSpawnPosition(double d, double d1, double d2) {
		prevPosX = posX = d;
		prevPosY = posY = d1;
		prevPosZ = posZ = d2;
		float f = width / 2.0F;
	...
	public void onEntityUpdate() {
		...
		if(isSprinting()) {
			...
			if(j1 > 0) {
				worldObj.spawnParticle((new StringBuilder()).append("tilecrack_").append(j1).toString(), posX + (rand.nextFloat() - 0.5D) * width, boundingBox.minY + 0.10000000000000001D, posZ + (rand.nextFloat() - 0.5D) * width, -motionX * 4D, 1.5D, -motionZ * 4D);
			}
		}
	...
	public void moveEntity(double d, double d1, double d2) {
		...
		if(flag) {
			...
			for(; d != 0.0D && (canCollideWithEntities ? worldObj.getCollidingBoundingBoxes(this, boundingBox.getOffsetBoundingBox(d, -1D, 0.0D)).size() : worldObj.getCollidingBlocks(boundingBox.getOffsetBoundingBox(d, -1D, 0.0D)).size()) == 0; d5 = d) {
				if(d < d8 && d >= -d8) {
					d = 0.0D;
					continue;
				}
			...
			for(; d2 != 0.0D && (canCollideWithEntities ? worldObj.getCollidingBoundingBoxes(this, boundingBox.getOffsetBoundingBox(0.0D, -1D, d2)).size() : worldObj.getCollidingBlocks(boundingBox.getOffsetBoundingBox(0.0D, -1D, d2)).size()) == 0; d7 = d2) {
				if(d2 < d8 && d2 >= -d8) {
					d2 = 0.0D;
					continue;
				}
		...
		List<AxisAlignedBB> list = canCollideWithEntities ? worldObj.getCollidingBoundingBoxes(this, boundingBox.addCoord(d, d1, d2)) : worldObj.getCollidingBlocks(boundingBox.addCoord(d, d1, d2));
		if(list.size() == 0) {
			boundingBox.offset(d, d1, d2);
		} else {
			...
			if(stepHeight > 0.0F && flag1 && (flag || ySize < 0.05F) && (d5 != d || d7 != d2)) {
				...
				List<AxisAlignedBB> list1 = canCollideWithEntities ? worldObj.getCollidingBoundingBoxes(this, boundingBox.addCoord(d, d1, d2)) : worldObj.getCollidingBlocks(boundingBox.addCoord(d, d1, d2));
				for(int j2 = 0; j2 < list1.size(); j2++)
					d1 = list1.get(j2).calculateYOffset(boundingBox, d1);
				boundingBox.offset(0.0D, d1, 0.0D);
				if(!field_9293_aM && d6 != d1)
					d = d1 = d2 = 0.0D;
				for(int k2 = 0; k2 < list1.size(); k2++)
					d = list1.get(k2).calculateXOffset(boundingBox, d);
				boundingBox.offset(d, 0.0D, 0.0D);
				if(!field_9293_aM && d5 != d)
					d = d1 = d2 = 0.0D;
				for(int l2 = 0; l2 < list1.size(); l2++)
					d2 = list1.get(l2).calculateZOffset(boundingBox, d2);
				...
				if(!field_9293_aM && d6 != d1) {
					...
					for(int i3 = 0; i3 < list1.size(); i3++)
						d1 = list1.get(i3).calculateYOffset(boundingBox, d1);
					boundingBox.offset(0.0D, d1, 0.0D);
				}
				if(d9 * d9 + d13 * d13 >= d * d + d2 * d2) {
					...
					double d14 = boundingBox.minY - (int) boundingBox.minY;
					if(d14 > 0.0D)
						ySize += d14 + 0.01D;
				}
			}
		...
		if(canTriggerWalking() && !flag && ridingEntity == null) {
			distanceWalkedModified += MathHelper.sqrt_double(d10 * d10 + d12 * d12) * 0.59999999999999998D;
			...
			if(distanceWalkedModified > nextStepDistance && j3 > 0) {
	...
	public int getEntityBrightnessForRender(float f) {
		...
		if(worldObj.blockExists(i, worldObj.worldSurface, j)) {
	...
	public void writeToNBT(NBTTagCompound nbttagcompound) {
		nbttagcompound.setTag("Pos", newDoubleNBTList(new double[]{posX, posY + ySize, posZ}));
	...
	public void readFromNBT(NBTTagCompound nbttagcompound) {
		...
		motionX = ((NBTTagDouble) nbttaglist1.get(0)).doubleValue;
		motionY = ((NBTTagDouble) nbttaglist1.get(1)).doubleValue;
		motionZ = ((NBTTagDouble) nbttaglist1.get(2)).doubleValue;
		...
		prevPosX = lastTickPosX = posX = ((NBTTagDouble) nbttaglist.get(0)).doubleValue;
		prevPosY = lastTickPosY = posY = ((NBTTagDouble) nbttaglist.get(1)).doubleValue;
		prevPosZ = lastTickPosZ = posZ = ((NBTTagDouble) nbttaglist.get(2)).doubleValue;
		prevRotationYaw = rotationYaw = ((NBTTagFloat) nbttaglist2.get(0)).floatValue;
		prevRotationPitch = rotationPitch = ((NBTTagFloat) nbttaglist2.get(1)).floatValue;
	...
	public EntityItem entityDropItem(ItemStack itemstack, float f) {
		EntityItem entityitem = new EntityItem(worldObj, posX, posY + f, posZ, itemstack);
	...
	public void mountEntity(Entity entity) {
		...
		if(ridingEntity == entity) {
			ridingEntity.riddenByEntity = null;
			ridingEntity = null;
			setLocationAndAngles(entity.posX, entity.boundingBox.minY + entity.height, entity.posZ, rotationYaw, rotationPitch);
			return;
		}
		if(ridingEntity != null) {
			ridingEntity.riddenByEntity = null;
