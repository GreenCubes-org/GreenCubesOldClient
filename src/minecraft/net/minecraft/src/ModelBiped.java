...
public class ModelBiped extends ModelBase {
	...
	public ModelRenderer bipedChatBalloon;
	...
	public ModelBiped(float f, float f1) {
		...
		bipedChatBalloon = new ModelRenderer(this, 0, 1);
		//bipedChatBalloon.addBox(-7.0F, 10.0F, -8.0F, 14, 1, 5, f);
	}

	@Override
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float tpf) {
		setRotationAngles(f, f1, f2, f3, f4, tpf);
		bipedHead.render(tpf);
		bipedBody.render(tpf);
		bipedRightArm.render(tpf);
		bipedLeftArm.render(tpf);
		bipedRightLeg.render(tpf);
		bipedLeftLeg.render(tpf);
		bipedHeadwear.render(tpf);
		if(isTyping) {
			bipedChatBalloon.render(tpf);
		}

	}

	@Override
	public void setRotationAngles(float f, float f1, float stayingAnimation, float headYaw, float headPitch, float f5) {
		bipedHead.rotateAngleY = headYaw / 57.29578F;
		...
		if(field_1279_h != 0) {
			bipedLeftArm.rotateAngleX = bipedLeftArm.rotateAngleX * 0.5F - 0.3141593F * field_1279_h;
		}
		if(field_1278_i != 0) {
			bipedRightArm.rotateAngleX = bipedRightArm.rotateAngleX * 0.5F - 0.3141593F * field_1278_i;
		}
		...
		bipedRightArm.rotateAngleZ += MathHelper.cos(stayingAnimation * 0.09F) * 0.05F + 0.05F;
		bipedLeftArm.rotateAngleZ -= MathHelper.cos(stayingAnimation * 0.09F) * 0.05F + 0.05F;
		bipedRightArm.rotateAngleX += MathHelper.sin(stayingAnimation * 0.067F) * 0.05F;
		bipedLeftArm.rotateAngleX -= MathHelper.sin(stayingAnimation * 0.067F) * 0.05F;
		if(field_40333_u) {
			float f7 = 0.0F;
			float f9 = 0.0F;
			bipedRightArm.rotateAngleZ = 0.0F;
