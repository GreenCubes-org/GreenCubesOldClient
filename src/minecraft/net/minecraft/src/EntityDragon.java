...
public class EntityDragon extends EntityDragonBase {
	...
	public void onLivingUpdate() {
		...
		if(health <= 0) {
			...
			worldObj.spawnParticle("largeexplode", posX + f, posY + 2D + f2, posZ + f4, 0.0D, 0.0D, 0.0D);
			return;
		}
		func_41011_ay();
		...
		if(worldObj.multiplayerWorld) {
			if(newPosRotationIncrements > 0) {
				...
				double d2 = posY + (newPosY - posY) / newPosRotationIncrements;
				double d4 = posZ + (newPosZ - posZ) / newPosRotationIncrements;
				double d6;
				for(d6 = newRotationYaw - rotationYaw; d6 < -180D; d6 += 360D) {
				}
				for(; d6 >= 180D; d6 -= 360D) {
				}
				rotationYaw += d6 / newPosRotationIncrements;
			...
			double d11;
			for(d11 = d9 - rotationYaw; d11 < -180D; d11 += 360D) {
			}
			...
			randomYawVelocity += d11 * (0.69999998807907104D / d14 / f19);
		...
		field_40171_aq.setLocationAndAngles(posX + (f8 * 0.5F), posY, posZ - (f9 * 0.5F), 0.0F, 0.0F);
		field_40175_au.onUpdate();
		field_40175_au.setLocationAndAngles(posX + (f9 * 4.5F), posY + 2D, posZ + (f8 * 4.5F), 0.0F, 0.0F);
		field_40174_av.onUpdate();
		field_40174_av.setLocationAndAngles(posX - (f9 * 4.5F), posY + 2D, posZ - (f8 * 4.5F), 0.0F, 0.0F);
		if(!worldObj.multiplayerWorld) {
			func_41007_az();
		}
		...
		field_40177_ap.setLocationAndAngles(posX + (f11 * 5.5F * f5), posY + (ad1[1] - ad[1]) * 1.0D + (f6 * 5.5F), posZ - (f12 * 5.5F * f5), 0.0F, 0.0F);
		for(int j = 0; j < 3; j++) {
			...
			float f17 = (j + 1) * 2.0F;
			dragonpart.onUpdate();
			dragonpart.setLocationAndAngles(posX - ((f8 * f16 + f14 * f17) * f5), ((posY + (ad2[1] - ad[1]) * 1.0D) - ((f17 + f16) * f6)) + 1.5D, posZ + ((f9 * f16 + f15 * f17) * f5), 0.0F, 0.0F);
		}
	...
	private void func_41011_ay() {
		if(field_41013_bH != null) {
			if(field_41013_bH.isDead) {
				...
				field_41013_bH = null;
			} else if(ticksExisted % 10 == 0 && health < field_40157_aB) {
				health++;
			}
		}
	...
	private void func_41006_aA() {
		...
		if(rand.nextInt(2) == 0 && worldObj.playerEntities.size() > 0) {
			field_40179_aC = worldObj.playerEntities.get(rand.nextInt(worldObj.playerEntities.size()));
		} else {
			boolean flag = false;
			do {
				field_40167_a = 0.0D;
	...
	private boolean func_40158_a(AxisAlignedBB axisalignedbb) {
		...
		if(flag1) {
			double d = axisalignedbb.minX + (axisalignedbb.maxX - axisalignedbb.minX) * rand.nextFloat();
			double d1 = axisalignedbb.minY + (axisalignedbb.maxY - axisalignedbb.minY) * rand.nextFloat();
			double d2 = axisalignedbb.minZ + (axisalignedbb.maxZ - axisalignedbb.minZ) * rand.nextFloat();
	...
	public boolean func_40156_a(DragonPart dragonpart, DamageSource damagesource, int i) {
		...
		field_40167_a = posX + (f1 * 5F) + ((rand.nextFloat() - 0.5F) * 2.0F);
		field_40165_b = posY + (rand.nextFloat() * 3F) + 1.0D;
		field_40166_c = (posZ - (f2 * 5F)) + ((rand.nextFloat() - 0.5F) * 2.0F);
		field_40179_aC = null;
	...
	protected void func_40120_m_() {
		...
		if(field_40178_aA >= 180 && field_40178_aA <= 200) {
			...
			worldObj.spawnParticle("hugeexplosion", posX + f, posY + 2D + f1, posZ + f2, 0.0D, 0.0D, 0.0D);
		}
