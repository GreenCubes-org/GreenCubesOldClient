...
public class RenderMinecart extends Render {
	...
	public void func_152_a(EntityMinecart entityminecart, double d, double d1, double d2, float f, float f1) {
		GL11.glPushMatrix();
		long l = entityminecart.entityId * 0x1d66f537L;
		l = l * l * 0x105cb26d1L + l * 0x181c9L;
		float f2 = (((l >> 16 & 7L) + 0.5F) / 8F - 0.5F) * 0.004F;
		float f3 = (((l >> 20 & 7L) + 0.5F) / 8F - 0.5F) * 0.004F;
		float f4 = (((l >> 24 & 7L) + 0.5F) / 8F - 0.5F) * 0.004F;
		GL11.glTranslatef(f2, f3, f4);
		double d3 = entityminecart.lastTickPosX + (entityminecart.posX - entityminecart.lastTickPosX) * f1;
		double d4 = entityminecart.lastTickPosY + (entityminecart.posY - entityminecart.lastTickPosY) * f1;
		double d5 = entityminecart.lastTickPosZ + (entityminecart.posZ - entityminecart.lastTickPosZ) * f1;
		...
		if(vec3d != null) {
			...
			if(vec3d3.lengthVector() != 0.0D) {
				...
				f = (float) ((TrigMath.atan2(vec3d3.zCoord, vec3d3.xCoord) * 180D) / 3.1415926535897931D);
				f5 = (float) (TrigMath.atan(vec3d3.yCoord) * 73D);
			}
		}
		GL11.glTranslatef((float) d, (float) d1, (float) d2);
		GL11.glRotatef(180F - f, 0.0F, 1.0F, 0.0F);
		GL11.glRotatef(-f5, 0.0F, 0.0F, 1.0F);
		float f6 = entityminecart.func_41023_l() - f1;
		float f7 = entityminecart.func_41025_i() - f1;
		if(f7 < 0.0F) {
			f7 = 0.0F;
		}
		if(f6 > 0.0F) {
			GL11.glRotatef(((MathHelper.sin(f6) * f6 * f7) / 10F) * entityminecart.func_41030_m(), 1.0F, 0.0F, 0.0F);
		}
