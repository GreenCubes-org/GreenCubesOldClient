	...
	public void onUpdate() {
		...
		if(onGround) {
			...
			if(worldObj.getBlockId(i, j, k) != Block.pistonMoving.blockID) {
				setEntityDead();
				if((!worldObj.canBlockBePlacedAt(blockID, i, j, k, true, 1) || BlockSand.canFallBelow(worldObj, i, j - 1, k) || !worldObj.setBlockAndMetadataWithNotify(i, j, k, blockID, blockData)) && !worldObj.multiplayerWorld) {
					entityDropItem(new ItemStack(blockID, 1, blockData), 0.0F);
				}
			}
