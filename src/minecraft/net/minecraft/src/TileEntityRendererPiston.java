...
public class TileEntityRendererPiston extends TileEntitySpecialRenderer {
	...
	public void func_31070_a(TileEntityPiston tileentitypiston, double d, double d1, double d2, float f) {
		...
		if(block != null && tileentitypiston.func_31008_a(f) < 1.0F) {
			...
			tessellator.setTranslationD(((float) d - tileentitypiston.xCoord) + tileentitypiston.func_31017_b(f), ((float) d1 - tileentitypiston.yCoord) + tileentitypiston.func_31014_c(f), ((float) d2 - tileentitypiston.zCoord) + tileentitypiston.func_31013_d(f));
			...
			if(block == Block.pistonExtension && tileentitypiston.func_31008_a(f) < 0.5F) {
				...
				tessellator.setTranslationD((float) d - tileentitypiston.xCoord, (float) d1 - tileentitypiston.yCoord, (float) d2 - tileentitypiston.zCoord);
