...
public class RenderFish extends Render {
	...
	public void doRenderFish(EntityFishHook entityfishhook, double d, double d1, double d2, float f, float f1) {
		GL11.glPushMatrix();
		...
		float f2 = (i * 8 + 0) / 128F;
		float f3 = (i * 8 + 8) / 128F;
		float f4 = (byte0 * 8 + 0) / 128F;
		float f5 = (byte0 * 8 + 8) / 128F;
		float f6 = 1.0F;
		float f7 = 0.5F;
		float f8 = 0.5F;
		...
		if(entityfishhook.angler != null) {
			...
			double d7 = entityfishhook.angler.prevPosX + (entityfishhook.angler.posX - entityfishhook.angler.prevPosX) * f1 + vec3d.xCoord;
			double d8 = entityfishhook.angler.prevPosY + (entityfishhook.angler.posY - entityfishhook.angler.prevPosY) * f1 + vec3d.yCoord;
			double d9 = entityfishhook.angler.prevPosZ + (entityfishhook.angler.posZ - entityfishhook.angler.prevPosZ) * f1 + vec3d.zCoord;
			if(renderManager.options.thirdPersonView > 0) {
				...
				d7 = (entityfishhook.angler.prevPosX + (entityfishhook.angler.posX - entityfishhook.angler.prevPosX) * f1) - d6 * 0.34999999999999998D - d4 * 0.84999999999999998D;
				d8 = (entityfishhook.angler.prevPosY + (entityfishhook.angler.posY - entityfishhook.angler.prevPosY) * f1) - 0.45000000000000001D;
				d9 = ((entityfishhook.angler.prevPosZ + (entityfishhook.angler.posZ - entityfishhook.angler.prevPosZ) * f1) - d4 * 0.34999999999999998D) + d6 * 0.84999999999999998D;
			}
			double d10 = entityfishhook.prevPosX + (entityfishhook.posX - entityfishhook.prevPosX) * f1;
			double d11 = entityfishhook.prevPosY + (entityfishhook.posY - entityfishhook.prevPosY) * f1 + 0.25D;
			double d12 = entityfishhook.prevPosZ + (entityfishhook.posZ - entityfishhook.prevPosZ) * f1;
			...
			for(int k = 0; k <= j; k++) {
				...
				tessellator.addVertex(d + d13 * f13, d1 + d14 * (f13 * f13 + f13) * 0.5D + 0.25D, d2 + d15 * f13);
			}
