...
public class BlockGreenStepGlass extends BlockGreenStepTextured {
	...
	public BlockGreenStepGlass(int i, int upTexture, int sideTexture, Block original, boolean up) {
		super(i, upTexture, sideTexture, original, up);
	}

	@Override
	public int getBlockTexture(IBlockAccess iblockaccess, int x, int y, int z, int i) {
		int d = iblockaccess.getBlockMetadata(x, y, z);
		int t = genericBlockTextureFromSideAndMetadata(i, d);
		if(t != super.getBlockTextureFromSideAndMetadata(i, d))
			return t;
		int texture = BlockGlassNew.glass0;
		boolean b[] = new boolean[6];
		if(i > 1) {
			switch(BlockFace.getFaceById(i)) {
			case EAST:
	...
	public int genericBlockTextureFromSideAndMetadata(int i, int j) {
		if(i == 0 || i == 1)
	...
	protected boolean isShouldBeConnected(IBlockAccess iblockaccess, int x, int y, int z, int i, int d) {
		...
		if(i == 0)
			...
			x2++;
		return ((iblockaccess.getBlockId(x, y, z) == blockID && iblockaccess.getBlockMetadata(x, y, z) == d) && !iblockaccess.isBlockOpaqueCube(x2, y2, z2) && (iblockaccess.getBlockId(x2, y2, z2) != blockID));
	}
}
