...
public abstract class Container {
	...
	protected List<Slot> playerRightSlots;
	...
	public Container() {
		...
		playerRightSlots = new ArrayList<Slot>(36);
		for(int i = 0; i < 36; ++i)
			playerRightSlots.add(null);
	}
	...
	public ItemStack slotClick(int slotId, int button, boolean shift, EntityPlayer player) {
		InventoryPlayer inventoryplayer = player.inventory;
		if(inventoryplayer.getItemStack() != null && inventoryplayer.getItemStack().isUndropable()) {
			if(slotId < 0)
				return inventoryplayer.getItemStack();
			Slot slot = inventorySlots.get(slotId);
			if(slot != null && slot.inventory != inventoryplayer)
				return inventoryplayer.getItemStack();
		}
		...
		if(button == 0 || button == 1) {
			if(slotId == -999) {
				...
				ItemStack itemstack1 = shiftTransfer(slotId);
				if(itemstack1 != null) {
					int k = itemstack1.itemID;
					itemstack = itemstack1.copy();
					Slot slot1 = inventorySlots.get(slotId);
					if(slot1 != null && slot1.getStack() != null && slot1.getStack().itemID == k) {
						returnItemAfterShiftClick(slotId, button, shift, player);
					}
				}
			} else {
				if(slotId < 0) {
				...
				if(slot != null) {
					...
					if(itemstack2 == null) {
						if(itemstack3 != null && slot.isItemValid(itemstack3)) {
							int l = button != 0 ? 1 : itemstack3.stackSize;
						...
						int i1 = button != 0 ? (itemstack2.stackSize + 1) / 2 : itemstack2.stackSize;
						...
						if(slot instanceof SlotTrashcan)
							itemstack2 = null;
						...
						if(!ItemStack.canStackItems(itemstack2, itemstack3) || itemstack2.itemID != itemstack3.itemID || itemstack2.getHasSubtypes() && itemstack2.getItemDamage() != itemstack3.getItemDamage()) {
							...
							int j1 = button != 0 ? 1 : itemstack3.stackSize;
	...
	protected void returnItemAfterShiftClick(int i, int j, boolean flag, EntityPlayer entityplayer) {
		...
		if(itemstack.isStackable()) {
			while(itemstack.stackSize > 0 && (!flag && k < j || flag && k >= i)) {
				...
				if(slot.isItemValid(itemstack)) {
					...
					if(itemstack1 != null && itemstack1.itemID == itemstack.itemID && ItemStack.canStackItems(itemstack1, itemstack) && (!itemstack.getHasSubtypes() || itemstack.getItemDamage() == itemstack1.getItemDamage())) {
	...
	protected boolean putToPlayerInvenoty(ItemStack itemstack, boolean flag) {
		boolean flag1 = false;
		int i = 0;
		int j = 36;
		int k = i;
		...
		if(itemstack.isStackable()) {
			while(itemstack.stackSize > 0 && (!flag && k < j || flag && k >= i)) {
				Slot slot = playerRightSlots.get(k);
				if(slot.isItemValid(itemstack)) {
					ItemStack itemstack1 = slot.getStack();
					if(itemstack1 != null && itemstack1.itemID == itemstack.itemID && ItemStack.canStackItems(itemstack1, itemstack) && (!itemstack.getHasSubtypes() || itemstack.getItemDamage() == itemstack1.getItemDamage())) {
						int i1 = itemstack1.stackSize + itemstack.stackSize;
		...
		if(itemstack.stackSize > 0) {
			...
			do {
				...
				Slot slot1 = playerRightSlots.get(l);
				if(slot1.isItemValid(itemstack)) {
					ItemStack itemstack2 = slot1.getStack();
					if(itemstack2 == null) {
						slot1.putStack(itemstack.copy());
