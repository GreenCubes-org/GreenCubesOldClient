...
public class ContainerFurnace extends Container {
	...
	public ContainerFurnace(InventoryPlayer inventoryplayer, TileEntityFurnace tileentityfurnace) {
		...
		for(int i = 0; i < 3; i++) {
			for(int k = 0; k < 9; k++) {
				Slot s = new Slot(inventoryplayer, k + i * 9 + 9, 8 + k * 18, 84 + i * 18);
				addSlot(s);
				playerRightSlots.set(i * 9 + k + 9, s);
			}

		}

		for(int j = 0; j < 9; j++) {
			Slot s = new Slot(inventoryplayer, j, 8 + j * 18, 142);
			addSlot(s);
			playerRightSlots.set(j, s);
		}
