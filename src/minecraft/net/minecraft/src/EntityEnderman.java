...
public class EntityEnderman extends EntityMob {
	...
	private boolean shouldAttackPlayer(EntityPlayer entityplayer) {
		...
		Vec3D vec3d1 = Vec3D.createVector(posX - entityplayer.posX, (boundingBox.minY + (height / 2.0F)) - (entityplayer.posY + entityplayer.getEyeHeight()), posZ - entityplayer.posZ);
	...
	protected boolean teleportToEntity(Entity entity) {
		Vec3D vec3d = Vec3D.createVector(posX - entity.posX, ((boundingBox.minY + (height / 2.0F)) - entity.posY) + entity.getEyeHeight(), posZ - entity.posZ);
		vec3d = vec3d.normalize();
		double d = 16D;
		double d1 = (posX + (rand.nextDouble() - 0.5D) * 8D) - vec3d.xCoord * d;
		double d2 = (posY + (rand.nextInt(16) - 8)) - vec3d.yCoord * d;
	...
	protected boolean teleportTo(double d, double d1, double d2) {
		...
		for(int j1 = 0; j1 < l; j1++) {
			...
			double d7 = d3 + (posX - d3) * d6 + (rand.nextDouble() - 0.5D) * width * 2D;
			double d8 = d4 + (posY - d4) * d6 + rand.nextDouble() * height;
			double d9 = d5 + (posZ - d5) * d6 + (rand.nextDouble() - 0.5D) * width * 2D;
	...
	static {
		canCarryBlocks = new boolean[Block.blocksLength]; // GreenCubes
