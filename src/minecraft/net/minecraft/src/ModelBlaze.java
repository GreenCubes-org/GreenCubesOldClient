...
public class ModelBlaze extends ModelBase {
	...
	public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5) {
		...
		for(int i = 0; i < 4; i++) {
			field_40323_a[i].rotationPointY = -2F + MathHelper.cos(((i * 2) + f2) * 0.25F);
		...
		for(int j = 4; j < 8; j++) {
			field_40323_a[j].rotationPointY = 2.0F + MathHelper.cos(((j * 2) + f2) * 0.25F);
		...
		for(int k = 8; k < 12; k++) {
			field_40323_a[k].rotationPointY = 11F + MathHelper.cos((k * 1.5F + f2) * 0.5F);
