...
public class BlockColoredStairsGlass extends BlockStairsGlass {
	
	public final int[][] colors;
	private int currentPass;
	private boolean renderingItem = false;

	public BlockColoredStairsGlass(int id, Block block, int[][] colors) {
		super(id, block);
		this.colors = colors;
	...
	public int getBlockTexture(IBlockAccess iblockaccess, int x, int y, int z, int i) {
		if(currentPass == 2)
			return colors[(iblockaccess.getBlockMetadata(x, y, z) >> 3) % colors.length][0];
	...
	public int colorMultiplier(IBlockAccess iblockaccess, int i, int j, int k) {
		return currentPass == 2 ? 0xffffff : getRenderColor(iblockaccess.getBlockMetadata(i, j, k) >> 3);
	}
