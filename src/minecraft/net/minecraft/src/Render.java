...
public abstract class Render {
	...
	protected void loadTexture(String s) {
		RenderEngine renderengine = Minecraft.theMinecraft.renderEngine;
	...
	private void renderEntityOnFire(Entity entity, double d, double d1, double d2, float f) {
		...
		float f1 = j / 256F;
		float f3 = (j + 15.99F) / 256F;
		float f5 = k / RenderEngine.TERRAIN_HEIGHT_F;
		float f7 = (k + 15.99F) / RenderEngine.TERRAIN_HEIGHT_F;
		GL11.glPushMatrix();
		...
		while(f12 > 0.0F) {
			...
			if(l % 2 == 0) {
				f2 = j / 256F;
				f4 = (j + 15.99F) / 256F;
				f6 = k / RenderEngine.TERRAIN_HEIGHT_F;
				f8 = (k + 15.99F) / RenderEngine.TERRAIN_HEIGHT_F;
			} else {
				f2 = j / 256F;
				f4 = (j + 15.99F) / 256F;
				f6 = (k + 16) / RenderEngine.TERRAIN_HEIGHT_F;
				f8 = ((k + 16) + 15.99F) / RenderEngine.TERRAIN_HEIGHT_F;
			}
			if((l / 2) % 2 == 0) {
				float f15 = f4;
				f4 = f2;
	...
	private void renderShadow(Entity entity, double d, double d1, double d2, float f, float f1) {
		...
		double d4 = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * f1 + entity.getShadowSize();
		...
		for(int k1 = i; k1 <= j; k1++) {
			for(int l1 = k; l1 <= l; l1++) {
				for(int i2 = i1; i2 <= j1; i2++) {
					if(world.isOutOfBounds(k1, l1 - 1, i2))
						continue;
					int j2 = world.getBlockId(k1, l1 - 1, i2);
					if(j2 > 0 && world.getBlockLightValue(k1, l1, i2) > 3) {
						renderShadowOnBlock(Block.blocksList[j2], d, d1 + entity.getShadowSize(), d2, k1, l1, i2, f, f2, d6, d7 + entity.getShadowSize(), d8);
					}
				}
	...
	private void renderShadowOnBlock(Block block, double d, double d1, double d2, int i, int j, int k, float f, float f1, double d3, double d4, double d5) {
		...
		double d6 = (f - (d1 - (j + d4)) / 2D) * 0.5D * getWorldFromRenderManager().getLightBrightness(i, j, k);
		if(d6 < 0.0D) {
			return;
		}
		if(d6 > 1.0D) {
		...
		double d7 = i + block.minX + d3;
		double d8 = i + block.maxX + d3;
		double d9 = j + block.minY + d4 + 0.015625D;
		double d10 = k + block.minZ + d5;
		double d11 = k + block.maxZ + d5;
		float f2 = (float) ((d - d7) / 2D / f1 + 0.5D);
		float f3 = (float) ((d - d8) / 2D / f1 + 0.5D);
		float f4 = (float) ((d2 - d10) / 2D / f1 + 0.5D);
		float f5 = (float) ((d2 - d11) / 2D / f1 + 0.5D);
	...
	public void doRenderShadowAndFire(Entity entity, double d, double d1, double d2, float f, float f1) {
		if(Minecraft.theMinecraft.gameSettings.fancyGraphics && shadowSize > 0.0F) {
			double d3 = renderManager.getDistanceToCamera(entity.posX, entity.posY, entity.posZ);
			float f2 = (float) ((1.0D - d3 / 256D) * field_194_c);
			if(f2 > 0.0F) {
