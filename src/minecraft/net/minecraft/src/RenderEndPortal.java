...
public class RenderEndPortal extends TileEntitySpecialRenderer {
	...
	public void func_40446_a(TileEntityEndPortal tileentityendportal, double d, double d1, double d2, float f) {
		...
		for(int i = 0; i < 16; i++) {
			...
			tessellator.addVertex(d, d1 + f4, d2 + 1.0D);
			tessellator.addVertex(d + 1.0D, d1 + f4, d2 + 1.0D);
			tessellator.addVertex(d + 1.0D, d1 + f4, d2);
			tessellator.draw();
			GL11.glPopMatrix();
