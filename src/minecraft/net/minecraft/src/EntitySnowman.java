...
public class EntitySnowman extends EntitySnowmanBase {
	...
	public void onLivingUpdate() {
		if(worldObj.multiplayerWorld) {
			this.prevRotationYaw = prevRenderYawOffset = renderYawOffset = this.field_704_R = this.field_705_Q = this.rotationYaw;
			return;
		}
		...
		for(int j = 0; j < 4; j++) {
			int l = MathHelper.floor_double(posX + (((j % 2) * 2 - 1) * 0.25F));
	...
	protected void attackEntity(Entity entity, float f) {
		if(f < 10F) {
			...
			if(attackTime == 0) {
				...
				double d2 = (entity.posY + entity.getEyeHeight()) - 1.1000000238418579D - entitysnowball.posY;
				float f1 = MathHelper.sqrt_double(d * d + d1 * d1) * 0.2F;
				worldObj.playSoundAtEntity(this, "random.bow", 1.0F, 1.0F / (rand.nextFloat() * 0.4F + 0.8F));
				worldObj.entityJoinedWorld(entitysnowball);
				entitysnowball.setThrowableHeading(d, d2 + f1, d1, 1.6F, 12F);
				attackTime = 10;
			}
