...
public class GChat {
	...
	private static final Pattern commandpattern = Pattern.compile("§7§7§7§1([^|]*)\\|?(.*)");
	...
	public void processOutput(String message) {
		...
		if(message.startsWith("~")) {
			...
			if(split[0].equals("bind")) {
				if(split.length < 3) {
					activeTab.addNewLine("Èñïîëüçîâàíèå: ~bind <êëàâèøà> <êîìàíäà...>");
				} else {
					...
					if(key == 0) {
						activeTab.addNewLine("Óêàçàííàÿ êëàâèøà íå íàéäåíà!");
					} else {
						bindings[sub].put(key, command);
						saveConfig();
						activeTab.addNewLine("Íîâîå ñî÷åòàíèå êëàâèø äîáàâëåíî!");
					}
				}
			} else if(split[0].equals("script")) {
				if(split.length < 2) {
					activeTab.addNewLine("Èñïîëüçîâàíèå: ~script <èìÿ ôàéëà...>");
				} else {
					String file = join(split, " ", 1);
					if(!executeScript(file)) {
						activeTab.addNewLine("\2474Ôàéë íå íàéäåí!");
					}
				}
			} else if(split[0].equals("unbind")) {
				if(split.length < 2) {
					activeTab.addNewLine("Èñïîëüçîâàíèå: ~unbind <êëàâèøà>");
				} else {
					int sub = 0;
					split[1] = split[1].toLowerCase();
					if(split[1].contains("shift-")) {
					...
					if(key == 0) {
						...
						saveConfig();
						activeTab.addNewLine("Ñî÷åòàíèå êëàâèø óäàëåíî!");
					}
				}
			} else if(split[0].equals("binds")) {
				activeTab.addNewLine("§aÒåêóùèå áèíäû:");
