...
public class ModelSilverfish extends ModelBase {
	...
	public ModelSilverfish() {
		...
		for(int i = 0; i < silverfishBodyParts.length; i++) {
			...
			silverfishBodyParts[i].addBox(silverfishBoxLength[i][0] * -0.5F, 0.0F, silverfishBoxLength[i][2] * -0.5F, silverfishBoxLength[i][0], silverfishBoxLength[i][1], silverfishBoxLength[i][2]);
		...
		silverfishWings[0].addBox(-5F, 0.0F, silverfishBoxLength[2][2] * -0.5F, 10, 8, silverfishBoxLength[2][2]);
		silverfishWings[0].setRotationPoint(0.0F, 16F, field_35399_c[2]);
		silverfishWings[1] = new ModelRenderer(this, 20, 11);
		silverfishWings[1].addBox(-3F, 0.0F, silverfishBoxLength[4][2] * -0.5F, 6, 4, silverfishBoxLength[4][2]);
		silverfishWings[1].setRotationPoint(0.0F, 20F, field_35399_c[4]);
		silverfishWings[2] = new ModelRenderer(this, 20, 18);
		silverfishWings[2].addBox(-3F, 0.0F, silverfishBoxLength[4][2] * -0.5F, 6, 5, silverfishBoxLength[1][2]);
	...
	public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5) {
		for(int i = 0; i < silverfishBodyParts.length; i++) {
			silverfishBodyParts[i].rotateAngleY = MathHelper.cos(f2 * 0.9F + i * 0.15F * 3.141593F) * 3.141593F * 0.05F * (1 + Math.abs(i - 2));
			silverfishBodyParts[i].rotationPointX = MathHelper.sin(f2 * 0.9F + i * 0.15F * 3.141593F) * 3.141593F * 0.2F * Math.abs(i - 2);
		}
