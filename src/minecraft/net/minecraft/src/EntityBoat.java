...
public class EntityBoat extends Entity {
	...
	public EntityBoat(World world, double d, double d1, double d2) {
		this(world);
		setSpawnPosition(d, d1 + yOffset, d2);
		motionX = 0.0D;
		motionY = 0.0D;
		motionZ = 0.0D;
		prevPosX = d;
	...
	public void onUpdate() {
		...
		double d = 0.0D;
		for(int j = 0; j < i; j++) {
			double d2 = (boundingBox.minY + ((boundingBox.maxY - boundingBox.minY) * (j + 0)) / i) - 0.125D;
		...
		if(d1 > 0.14999999999999999D) {
			double d3 = Math.cos((rotationYaw * 3.1415926535897931D) / 180D);
			double d9 = Math.sin((rotationYaw * 3.1415926535897931D) / 180D);
			for(int i1 = 0; i1 < 1.0D + d1 * 60D; i1++) {
				double d16 = rand.nextFloat() * 2.0F - 1.0F;
				double d19 = (rand.nextInt(2) * 2 - 1) * 0.69999999999999996D;
				if(rand.nextBoolean()) {
		...
		if(worldObj.multiplayerWorld) {
			if(boatPosRotationIncrements > 0) {
				double d4 = posX + (boatX - posX) / boatPosRotationIncrements;
				double d10 = posY + (boatY - posY) / boatPosRotationIncrements;
				double d13 = posZ + (boatZ - posZ) / boatPosRotationIncrements;
				double d17;
				for(d17 = boatYaw - rotationYaw; d17 < -180D; d17 += 360D) {
				}
				for(; d17 >= 180D; d17 -= 360D) {
				}
				rotationYaw += d17 / boatPosRotationIncrements;
		...
		double d20;
		for(d20 = d12 - rotationYaw; d20 >= 180D; d20 -= 360D) {
		}
		...
		for(int k1 = 0; k1 < 4; k1++) {
			int l1 = MathHelper.floor_double(posX + ((k1 % 2) - 0.5D) * 0.80000000000000004D);
			int i2 = MathHelper.floor_double(posY);
			int j2 = MathHelper.floor_double(posZ + ((k1 / 2) - 0.5D) * 0.80000000000000004D);
