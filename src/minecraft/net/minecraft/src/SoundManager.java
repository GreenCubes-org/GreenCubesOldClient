...
public class SoundManager {
	...
	private List<ScheduledSound> scheduledSounds = new ArrayList<ScheduledSound>();
	...
	public void func_338_a(EntityLiving entityliving, float f) {
		...
		if(entityliving == null) {
			...
			double d1 = entityliving.prevPosY + (entityliving.posY - entityliving.prevPosY) * f;
	...
	public void updateSoundManager() {
		if(!this.scheduledSounds.isEmpty()) {
			for(int i = 0; i < scheduledSounds.size(); ++i) {
				ScheduledSound var2 = scheduledSounds.get(i);
				var2.delayTicks--;
				if(var2.delayTicks <= 0) {
					this.playSound(var2.sound, var2.x, var2.y, var2.z, var2.volume, var2.pitch);
					scheduledSounds.remove(i--);
				}
			}
		}
	}

	public void scheduleSound(String s, float x, float y, float z, float volume, float pitch, int delayTicks) {
		this.scheduledSounds.add(new ScheduledSound(s, x, y, z, volume, pitch, delayTicks));
	}
}
