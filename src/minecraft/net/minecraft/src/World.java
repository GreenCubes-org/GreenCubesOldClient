...
public class World implements IBlockAccess {
	 ...
	 */
	protected static final int DIST_HASH_MAGIC = 0x3c6ef35f;

	/*
	 * World format constants
	 */
	public static int metadataLength = 8;
	public static int metadataMult = 0xFF;
	public static int chunkHeight = 128;
	public static int chunkMult = 0x7F;
	...
	/**
	 * Current random
	 * (Do we really need this? May be replace with random?)
	 */
	protected int updateLCG = new Random().nextInt();

	/*
	 * World current height properties
	 */
	public int maxWorldHeight = 127;
	public int worldSurface = 64;
	public int maxWorldHeightChunks = (maxWorldHeight + 1) / chunkHeight;
	/*
	 * Entities
	 */
	public List<Entity> loadedEntityList = new FastList<Entity>();
	private List<Entity> unloadedEntityList = new FastList<Entity>();
	public List<EntityPlayer> playerEntities = new FastList<EntityPlayer>();
	public List<Entity> weatherEffects = new FastList<Entity>();
	/*
	 * Tile entities to tick
	 */
	public List<TileEntity> loadedTileEntityList = new FastList<TileEntity>();
	private List<TileEntity> addedTileEntityList = new FastList<TileEntity>();
	private List<TileEntity> toRemoveTileEntityList = new FastList<TileEntity>();
	/*
	 * Block ticks
	 */
	private TreeSet<NextTickListEntry> scheduledTickTreeSet = new TreeSet<NextTickListEntry>();
	private Set<NextTickListEntry> scheduledTickSet = new THashSet<NextTickListEntry>();
	private Set<ChunkCoordIntPair> positionsToUpdate = new THashSet<ChunkCoordIntPair>();

	/*
	 * Caches
	 */
	private FastList<Entity> entityCache = new FastList<Entity>(20);
	private ArrayList<AxisAlignedBB> collidingBoxesCache = new ArrayList<AxisAlignedBB>(40);

	public int field_35473_a = 7;
	public int field_35471_b = field_35473_a + 4;
	public int field_35472_c = 1 << field_35473_a;
	public int field_35469_d = field_35472_c - 1;
	public int field_35470_e = field_35472_c / 2 - 1;
	...
	protected List<IWorldAccess> worldAccesses = new FastList<IWorldAccess>();
	protected boolean spawnHostileMobs = true;
	protected boolean spawnPeacefulMobs = true;

	private int soundCounter = rand.nextInt(12000);
	...
	public World(ISaveHandler isavehandler, String s, WorldSettings worldsettings) {
		this(isavehandler, s, worldsettings, null);
	}
	...
	public void setBlockMetadataWithNotify(int i, int j, int k, int l) {
		if(setBlockMetadata(i, j, k, l)) {
			...
			if(Block.requiresSelfNotify[i1]) {
	...
	public void markBlockNeedsUpdate(int i, int j, int k) {
		for(int l = 0; l < worldAccesses.size(); l++) {
			worldAccesses.get(l).markBlockAndNeighborsNeedsUpdate(i, j, k);
		}
	...
	public void markBlockAsNeedsUpdate(int i, int j, int k) {
		for(int l = 0; l < worldAccesses.size(); l++) {
			worldAccesses.get(l).markBlockRangeNeedsUpdate(i, j, k, i, j, k);
		}
	...
	public void markBlocksDirty(int i, int j, int k, int l, int i1, int j1) {
		for(int k1 = 0; k1 < worldAccesses.size(); k1++) {
			worldAccesses.get(k1).markBlockRangeNeedsUpdate(i, j, k, l, i1, j1);
		}
	...
	public void setLightValue(EnumSkyBlock enumskyblock, int i, int j, int k, int l) {
		...
		for(int i1 = 0; i1 < worldAccesses.size(); i1++) {
			worldAccesses.get(i1).markBlockAndNeighborsNeedsUpdate(i, j, k);
		}
	...
	public MovingObjectPosition rayTraceBlocks_do_do(Vec3D vec3d, Vec3D vec3d1, boolean traceLiquids, boolean flag1) {
		...
		if((!flag1 || block == null || block.getCollisionBoundingBoxFromPool(this, l, i1, j1) != null) && k1 > 0 && block.canCollideCheck(i2, traceLiquids)) {
		...
		for(int l1 = 200; l1-- >= 0;) {
			...
			try {
				...
				if((!flag1 || block1 == null || block1.getCollisionBoundingBoxFromPool(this, l, i1, j1) != null) && j2 > 0 && block1.canCollideCheck(k2, traceLiquids)) {
				...
				System.out.println("Block ID is: " + j2); //
				System.out.println("Coordinates: " + l + ", " + i1 + ", " + j1);
				System.out.println("Chunk: " + (l >> 4) + ", " + (j1 >> 4));
				System.out.println("Block by chunk: " + getChunkFromChunkCoords(l >> 4, j1 >> 4).getBlockID(l & 0xF, i1, j1 & 0xF) + ", " + getChunkFromChunkCoords(l >> 4, j1 >> 4).blocks[(l & 0xF) << field_35471_b | (j1 & 0xF) << field_35473_a | i1] + ", " + getChunkFromChunkCoords(l >> 4, j1 >> 4).addData.getNibble(l & 0xF, i1, j1 & 0xF));
				throw e;
			}
		}
	...
	public void playSoundAtEntity(Entity entity, String s, float f, float f1) {
		for(int i = 0; i < worldAccesses.size(); i++) {
			worldAccesses.get(i).playSound(s, entity.posX, entity.posY - entity.yOffset, entity.posZ, f, f1);
		}
	...
	public void playSoundEffect(double d, double d1, double d2, String s, float f, float f1) {
		for(int i = 0; i < worldAccesses.size(); i++) {
			worldAccesses.get(i).playSound(s, d, d1, d2, f, f1);
		}
	...
	public void playRecord(String s, int i, int j, int k) {
		for(int l = 0; l < worldAccesses.size(); l++) {
			worldAccesses.get(l).playRecord(s, i, j, k);
		}
	...
	public void spawnParticle(String s, double d, double d1, double d2, double d3, double d4, double d5) {
		for(int i = 0; i < worldAccesses.size(); i++) {
			worldAccesses.get(i).spawnParticle(s, d, d1, d2, d3, d4, d5);
		}
	}

	public void playSound(double x, double y, double z, String sound, float volume, float pitch, boolean delayed) {
		float var11 = 16.0F;
		...
		double var12 = Minecraft.theMinecraft.renderViewEntity.getDistanceSq(x, y, z);

		if(var12 < var11 * var11) {
			if(delayed && var12 > 100.0D) {
				double var14 = Math.sqrt(var12) / 40.0D;
				Minecraft.theMinecraft.sndManager.scheduleSound(sound, (float) x, (float) y, (float) z, volume, pitch, (int) Math.round(var14 * 20.0D));
			} else {
				Minecraft.theMinecraft.sndManager.playSound(sound, (float) x, (float) y, (float) z, volume, pitch);
			}
		}
	}
	...
	protected void obtainEntitySkin(Entity entity) {
		for(int i = 0; i < worldAccesses.size(); i++) {
			worldAccesses.get(i).obtainEntitySkin(entity);
		}
	...
	protected void releaseEntitySkin(Entity entity) {
		for(int i = 0; i < worldAccesses.size(); i++) {
			worldAccesses.get(i).releaseEntitySkin(entity);
		}
	...
	public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entity, AxisAlignedBB axisalignedbb) {
		...
		for(int k1 = i; k1 < j; k1++) {
			for(int l1 = i1; l1 < j1; l1++) {
				if(!chunkExists(k1 >> 4, l1 >> 4))
					continue;
				Chunk c = getChunkFromBlockCoords(k1, l1);
				for(int i2 = k - 1; i2 < l; i2++) {
					if(i2 < 0 || i2 > maxWorldHeight)
						continue;
					Block block = Block.blocksList[c.getBlockID(k1 & 0xF, i2 & chunkMult, l1 & 0xF)];
					if(block != null)
						block.getCollidingBoundingBoxes(this, k1, i2, l1, axisalignedbb, collidingBoxesCache);
				}
			}
		}
		...
		for(int j2 = 0; j2 < list.size(); j2++) {
			...
			if(axisalignedbb1 != null && axisalignedbb1.intersectsWith(axisalignedbb))
				collidingBoxesCache.add(axisalignedbb1);
			axisalignedbb1 = entity.getCollisionBox(list.get(j2));
			if(axisalignedbb1 != null && axisalignedbb1.intersectsWith(axisalignedbb))
				collidingBoxesCache.add(axisalignedbb1);
		}

		return collidingBoxesCache;
	}

	public List<AxisAlignedBB> getCollidingBlocks(AxisAlignedBB axisalignedbb) {
		collidingBoxesCache.clear();
		int i = MathHelper.floor_double(axisalignedbb.minX);
		int j = MathHelper.floor_double(axisalignedbb.maxX + 1.0D);
		int k = MathHelper.floor_double(axisalignedbb.minY);
		...
		for(int k1 = i; k1 < j; k1++) {
			for(int l1 = i1; l1 < j1; l1++) {
				...
				for(int i2 = k - 1; i2 < l; i2++) {
					if(i2 < 0 || i2 > maxWorldHeight)
						continue;
					Block block = Block.blocksList[c.getBlockID(k1 & 0xF, i2 & World.chunkMult, l1 & 0xF)];
					if(block != null)
						block.getCollidingBoundingBoxes(this, k1, i2, l1, axisalignedbb, collidingBoxesCache);
				}
			}
	...
	public int calculateSkylightSubtracted(float f) {
		...
		f2 = 1.0F - f2;
		f2 = (float) (f2 * (1.0D - (getRainStrength(f) * 5F) / 16D));
		f2 = (float) (f2 * (1.0D - (getWeightedThunderStrength(f) * 5F) / 16D));
		f2 = 1.0F - f2;
		return (int) (f2 * 11F);
	}
	...
	public Vec3D getSkyColor(Entity entity, float f) {
		...
		int k = getBiomeAt(i, 127, j).getSkyColorByTemp(f3);
		float f4 = (k >> 16 & 0xff) / 255F;
		float f5 = (k >> 8 & 0xff) / 255F;
		float f6 = (k & 0xff) / 255F;
		f4 *= f2;
		f5 *= f2;
		f6 *= f2;
	...
	public Vec3D drawClouds(float f) {
		...
		float f3 = (cloudColour >> 16 & 255L) / 255F;
		float f4 = (cloudColour >> 8 & 255L) / 255F;
		float f5 = (cloudColour & 255L) / 255F;
	...
	public void updateEntities() {
		...
		for(int j = 0; j < unloadedEntityList.size(); j++) {
			Entity entity1 = unloadedEntityList.get(j);
		...
		for(int k = 0; k < unloadedEntityList.size(); k++) {
			releaseEntitySkin(unloadedEntityList.get(k));
		}

		unloadedEntityList.clear();
		Profiler.endStartSection("regular");
		for(int l = 0; l < loadedEntityList.size(); l++) {
			Entity entity2 = loadedEntityList.get(l);
		...
		if(!toRemoveTileEntityList.isEmpty()) {
			loadedTileEntityList.removeAll(toRemoveTileEntityList);
			toRemoveTileEntityList.clear();
		}
	...
	public boolean checkIfAABBIsClear(AxisAlignedBB axisalignedbb) {
		List list = getEntitiesWithinAABBExcludingEntity(null, axisalignedbb); // Use FX 'cause it's a bit faster
		...
		for(int k1 = i; k1 < j; k1++) {
			for(int l1 = k; l1 < l; l1++) {
				for(int i2 = i1; i2 < j1; i2++) {
					...
					double d1 = (l1 + 1) - BlockFluid.getFluidHeightPercent(getBlockMetadata(k1, l1, i2));
					if(l >= d1) {
						flag = true;
	...
	public void func_35455_a(TileEntity tileentity) {
		toRemoveTileEntityList.add(tileentity);
	}
	...
	public void tick() {
		...
		Profiler.startSection("biomeFlush");
		...
		for(Iterator iterator = positionsToUpdate.iterator(); iterator.hasNext(); Profiler.endSection()) {
			...
			if(soundCounter == 0) {
				...
				if(j6 == 0 && getFullBlockLightValue(j3, j5, j4) <= rand.nextInt(8) && getSavedLightValue(EnumSkyBlock.Sky, j3, j5, j4) <= 0) {
					EntityPlayer entityplayer1 = getClosestPlayer(j3 + 0.5D, j5 + 0.5D, j4 + 0.5D, 8D);
					if(entityplayer1 != null && entityplayer1.getDistanceSq(j3 + 0.5D, j5 + 0.5D, j4 + 0.5D) > 4D) {
						playSoundEffect(j3 + 0.5D, j5 + 0.5D, j4 + 0.5D, "ambient.cave.cave", 0.7F, 0.8F + rand.nextFloat() * 0.2F);
			...
			for(int i3 = 0; i3 < 20; i3++) {
				...
				int l6 = chunk.getBlockID(i5, k6, i6);
				k++;
	...
	public boolean tickUpdates(boolean flag) {
		...
		for(int j = 0; j < i; j++) {
			NextTickListEntry nextticklistentry = scheduledTickTreeSet.first();
	...
	public void randomDisplayUpdates(int i, int j, int k) {
		...
		for(int l = 0; l < 1000; l++) {
			...
			if(rand.nextInt(8) > j1 && l1 == 0) {
				spawnParticle("depthsuspend", i1 + rand.nextFloat(), j1 + rand.nextFloat(), k1 + rand.nextFloat(), 0.0D, 0.0D, 0.0D);
			}
	...
	public List<Entity> getEntitiesWithinAABBExcludingEntity(Entity entity, AxisAlignedBB axisalignedbb) {
		entityCache.clear();
		int i = MathHelper.floor_double(axisalignedbb.minX - 2D) >> 4;
		int j = MathHelper.floor_double(axisalignedbb.maxX + 2D) >> 4;
		int k = MathHelper.floor_double(axisalignedbb.minZ - 2D) >> 4;
		int l = MathHelper.floor_double(axisalignedbb.maxZ + 2D) >> 4;
		for(int i1 = i; i1 <= j; i1++)
			for(int j1 = k; j1 <= l; j1++)
				if(chunkExists(i1, j1))
					getChunkFromChunkCoords(i1, j1).getEntitiesWithinAABBForEntity(entity, axisalignedbb, entityCache);
		return entityCache;
	}
	...
	public List<Entity> getEntitiesWithinAABB(Class class1, AxisAlignedBB axisalignedbb) {
		...
		int k = MathHelper.floor_double(axisalignedbb.minZ - 2D) >> 4;
		int l = MathHelper.floor_double(axisalignedbb.maxZ + 2D) >> 4;
		List<Entity> arraylist = new FastList<Entity>();
		for(int i1 = i; i1 <= j; i1++)
			for(int j1 = k; j1 <= l; j1++)
				if(chunkExists(i1, j1))
	...
	public void updateTileEntityChunkAndDoNothing(int i, int j, int k, TileEntity tileentity) {
		...
		for(int l = 0; l < worldAccesses.size(); l++)
			worldAccesses.get(l).doNothingWithTileEntity(i, j, k, tileentity);
	}

	public int countEntities(Class class1) {
		int i = 0;
		for(int j = 0; j < loadedEntityList.size(); j++) {
			Entity entity = loadedEntityList.get(j);
	...
	public boolean canBlockBePlacedAt(int i, int j, int k, int l, boolean flag, int i1) {
		...
		AxisAlignedBB axisalignedbb = flag ? null : block1.getCollisionBoundingBoxFromPool(this, j, k, l);
		if(axisalignedbb != null && !checkIfAABBIsClear(axisalignedbb))
			return false;
		if(block == Block.waterMoving || block == Block.waterStill || block == Block.lavaMoving || block == Block.lavaStill || block == Block.fire || block == Block.snow || block == Block.vine || block == Block.tallGrass) {
			block = null;
		}
	...
	public EntityPlayer getPlayerEntityByName(String s) {
		for(int i = 0; i < playerEntities.size(); i++) {
			if(s.equals(playerEntities.get(i).username)) {
				return playerEntities.get(i);
			}
		}
	...
	public int addFullChunkData(int x, int z, byte data[], int start) {
		int i = getChunkFromChunkCoords(x, z).addData(data, start);
		markBlocksDirty(x * 16, 0, z * 16, x * 16 + 15, 127, z * 16 + 15);
		return i;
	}

	public void setFullChunkData(int x, int z, byte data[]) {
		getChunkFromChunkCoords(x, z).setData(data);
		markBlocksDirty(x * 16, 0, z * 16, x * 16 + 15, 127, z * 16 + 15);
	}
	...
	public void updateEntityList() {
		...
		for(int j = 0; j < unloadedEntityList.size(); j++) {
			releaseEntitySkin(unloadedEntityList.get(j));
		}

		unloadedEntityList.clear();
		for(int k = 0; k < loadedEntityList.size(); k++) {
			Entity entity1 = loadedEntityList.get(k);
	...
	public boolean getIsThundering() {
		return getWeightedThunderStrength(1.0F) > 0.90000000000000002D;
	}

	public boolean isRaining() {
		return getRainStrength(1.0F) > 0.20000000000000001D;
	}
	...
	public boolean canLightningStrikeAt(int i, int j, int k) {
		...
		BiomeGenBase biomegenbase = getBiomeAt(i, j, k);
	...
	public void playAuxSFXAtEntity(EntityPlayer entityplayer, int i, int j, int k, int l, int i1) {
		for(int j1 = 0; j1 < worldAccesses.size(); j1++) {
			worldAccesses.get(j1).playAuxSFX(entityplayer, i, j, k, l, i1);
		}
	...
	public Random setRandomSeed(int i, int j, int k) {
		long l = i * 0x4f9939f508L + j * 0x1ef1565bd5L + getWorldInfo().getRandomSeed() + k;
		rand.setSeed(l);
		return rand;
	}
	...
	public BiomeGenBase getBiomeAt(int x, int y, int z) {
		if(blockExists(x, 0, z)) {
			...
			if(chunk != null)
				return chunk.getBiome(x & 0xF, z & 0xF, worldProvider.worldChunkMgr);
		}
		return worldProvider.worldChunkMgr.getBiomeGenAt(x, z);
	}
