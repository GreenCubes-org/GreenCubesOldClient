...
public class GuiMainMenu extends GuiScreen
{
    ...
    private void func_35355_b(int i, int j, float f)
    {
        ...
        for(int l = 0; l < k * k; l++)
        {
            ...
            GL11.glRotatef(MathHelper.sin((field_35357_f + f) / 400F) * 25F + 20F, 1.0F, 0.0F, 0.0F);
            GL11.glRotatef(-(field_35357_f + f) * 0.1F, 0.0F, 1.0F, 0.0F);
    ...
    private void renderSkybox(int i, int j, float f)
    {
        ...
        Tessellator tessellator = Tessellator.instance;
        tessellator.startDrawingQuads();
        float f1 = width <= height ? 120F / height : 120F / width;
        float f2 = (height * f1) / 256F;
        float f3 = (width * f1) / 256F;
	...
	public void drawScreen(int i, int j, float f)
    {
        ...
        float f1 = 1.8F - MathHelper.abs(MathHelper.sin(((System.currentTimeMillis() % 1000L) / 1000F) * 3.141593F * 2.0F) * 0.1F);
        f1 = (f1 * 100F) / (fontRenderer.getStringWidth(splashText) + 32);
        GL11.glScalef(f1, f1, f1);
        drawCenteredString(fontRenderer, splashText, 0, -8, 0xffff00);
        GL11.glPopMatrix();
        // GreenCubes start
        drawString(fontRenderer, "Специально для \2472Green\247fCubes", 2, height - 10, 0xffffff);
