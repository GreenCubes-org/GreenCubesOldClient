...
public class ChunkProviderGenerate implements IChunkProvider {
		...
		if(field_35388_l == null) {
			...
			for(int k1 = -2; k1 <= 2; k1++) {
				for(int l1 = -2; l1 <= 2; l1++) {
					float f = 10F / MathHelper.sqrt_float((k1 * k1 + l1 * l1) + 0.2F);
		...
		for(int k2 = 0; k2 < l; k2++) {
			for(int l2 = 0; l2 < j1; l2++) {
				...
				for(int k3 = 0; k3 < i1; k3++) {
					...
					d3 = (d3 * i1) / 16D;
					double d5 = i1 / 2D + d3 * 4D;
					double d6 = 0.0D;
					double d7 = ((k3 - d5) * 12D * 128D) / worldObj.field_35472_c / d4;
					if(d7 < 0.0D) {
						d7 *= 4D;
					}
	...
	public void populate(IChunkProvider ichunkprovider, int i, int j) {
		...
		int k = i * 16;
		int l = j * 16;
		BiomeGenBase biomegenbase = worldObj.getBiomeAt(k + 16, 127, l + 16);
		rand.setSeed(worldObj.getWorldSeed());
		long l1 = (rand.nextLong() / 2L) * 2L + 1L;
		long l2 = (rand.nextLong() / 2L) * 2L + 1L;
		rand.setSeed(i * l1 + j * l2 ^ worldObj.getWorldSeed());
		boolean flag = false;
		if(mapFeaturesEnabled) {
	...
	public List func_40377_a(EnumCreatureType enumcreaturetype, int i, int j, int k) {
		...
		BiomeGenBase biomegenbase = worldObj.getBiomeAt(i, j, k);
