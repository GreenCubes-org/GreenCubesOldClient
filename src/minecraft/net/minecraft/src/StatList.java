...
public class StatList {
	...
	private static StatBase[] initMinableStats(String s, int i) {
		StatBase astatbase[] = new StatBase[Block.blocksList.length]; // GreenCubes
		for(int j = 0; j < Block.blocksList.length; j++) // GreenCubes
		{
			if(Block.blocksList[j] != null && Block.blocksList[j].getEnableStats()) {
				String s1 = StatCollector.translateToLocalFormatted(s, new Object[]{Block.blocksList[j].translateBlockName()});
				astatbase[j] = (new StatCrafting(i + j, s1, j)).registerStat();
				objectMineStats.add(astatbase[j]);
			}
		}
	...
	private static StatBase[] initUsableStats(StatBase astatbase[], String s, int i, int j, int k) {
		...
		for(int l = j; l < k; l++) {
			...
			String s1 = StatCollector.translateToLocalFormatted(s, new Object[]{Item.itemsList[l].getStatName()});
			astatbase[l] = (new StatCrafting(i + l, s1, l)).registerStat();
			if(l >= Block.blocksList.length) {
				itemStats.add(astatbase[l]);
			}
		}
