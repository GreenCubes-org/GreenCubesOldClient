...
public class EntityWolf extends EntityAnimal {
	...
	protected void updateEntityActionState() {
		...
		} else if(entityToAttack == null && !hasPath() && !isWolfTamed() && worldObj.rand.nextInt(100) == 0) {
		...
		if(hasCurrentTarget() && !hasPath() && !isWolfAngry()) {
			...
			if(entity instanceof EntityPlayer) {
				...
				if(itemstack != null) {
					...
					} else if(isWolfTamed() && (Item.itemsList[itemstack.itemID] instanceof ItemFood)) {
	...
	protected void attackEntity(Entity entity, float f) {
		...
		} else if(f < 1.5D && entity.boundingBox.maxY > boundingBox.minY && entity.boundingBox.minY < boundingBox.maxY) {
			attackTime = 20;
			byte byte0 = 2;
			if(isWolfTamed()) {
				byte0 = 4;
	...
	public float setTailRotation() {
		...
		if(isWolfTamed()) {
			return (0.55F - (20 - dataWatcher.getWatchableObjectInt(18)) * 0.02F) * 3.141593F;
		} else {
			return 0.6283185F;
		}
	}
